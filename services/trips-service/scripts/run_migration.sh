#!/bin/bash
# services/trips-service/scripts/run_migration.sh
# Run database migrations for trips-service
...
DATABASE_URL=${DATABASE_URL:-"postgresql://dev:dev123@localhost:5437/trips_db?sslmode=disable"}
DIRECTION=${1:-"up"}  # up, down, or version number

echo -e "${YELLOW}📦 Running database migrations for trips-service...${NC}"
echo ""
