...
func (m *MockTripRepositoryWrite) UpdateAvailableSeats(ctx context.Context, tripID string, newAvailableSeats int16) error {
	args := m.Called(ctx, tripID, newAvailableSeats)
	return args.Error(0)
}

func (m *MockTripRepositoryWrite) CancelTrip(ctx context.Context, tripID, driverID, reason string) error {
	args := m.Called(ctx, tripID, driverID, reason)
	return args.Error(0)
}
