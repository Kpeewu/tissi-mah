#!/bin/bash
# services/booking-service/scripts/generate_proto.sh
# Generate protobuf files for booking-service
...
echo -e "${YELLOW}🔧 Generating protobuf files for booking-service...${NC}"
...
if [ ! -f "${PROTO_DIR}/booking.proto" ]; then
  echo -e "${RED}❌ No proto files found in ${PROTO_DIR}/${NC}"
  exit 1
fi

# Generate booking.proto
echo ""
