#!/bin/bash
# services/email-service/scripts/generate_proto.sh
# Generate protobuf files for email-service
...
echo -e "${YELLOW}🔧 Generating protobuf files for email-service...${NC}"
...
if [ ! -f "${PROTO_DIR}/email.proto" ]; then
  echo -e "${RED}❌ No proto files found in ${PROTO_DIR}/${NC}"
  exit 1
fi

# Generate email.proto
echo ""
echo "Generating Go code from email.proto..."
protoc \
