#!/bin/bash
# services/vehicle-service/scripts/generate_proto.sh
# Generate protobuf files for vehicle-service
...
echo -e "${YELLOW}🔧 Generating protobuf files for vehicle-service...${NC}"
...
if [ ! -f "${PROTO_DIR}/vehicle.proto" ]; then
  echo -e "${RED}❌ No proto files found in ${PROTO_DIR}/${NC}"
  exit 1
fi

# Generate vehicle.proto
echo ""
