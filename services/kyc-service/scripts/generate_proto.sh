#!/bin/bash
# services/kyc-service/scripts/generate_proto.sh
# Generate protobuf files for kyc-service
...
echo -e "${YELLOW}Generating protobuf files for kyc-service...${NC}"
...
if [ ! -f "${GOOGLE_API_DIR}/annotations.proto" ]; then
  ...
  echo -e "${GREEN}Downloaded google/api proto files${NC}"
fi

# Check if proto files exist
if [ ! -f "${PROTO_DIR}/kyc.proto" ]; then
