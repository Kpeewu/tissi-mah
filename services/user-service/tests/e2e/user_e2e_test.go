...
func TestE2E_ChangeProfilePicture(t *testing.T) {
	t.Run("succes - photo uploadee et profil retourne", func(t *testing.T) {
		cleanCollection(t)

		// Créer un utilisateur en base
		createResp, err := grpcClient.CreateUser(context.Background(), &userpb.CreateUserRequest{
			AuthID: "auth-photo-e2e", FirebaseID: "firebase-photo-e2e",
			Name: "Diallo", FirstName: "Mamadou",
		})
		require.NoError(t, err)
		userID := createResp.UserID

		imageBytes := []byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10}
		imageURL := "https://s3.amazonaws.com/profile-photo-e2e.jpg"

		mockFileClient.On("UploadProfilePicture", mock.Anything, userID, imageBytes).Return(imageURL, nil).Once()
		mockFileClient.On("GetDocumentExpiry", mock.Anything, mock.Anything, mock.Anything).Return("").Maybe()
		mockAuthClient.On("GetAuthInfo", mock.Anything, "auth-photo-e2e").Return(&client.AuthInfo{
			AuthID: "auth-photo-e2e", Email: "mamadou@example.com", PhoneNumber: "+22591000000", IsActive: true,
		}, nil).Once()

		resp, err := grpcClient.ChangeProfilePicture(ctxWithUID("firebase-photo-e2e"), &userpb.ChangeProfilePictureRequest{
			UserID:            userID,
		...
		assert.Equal(t, imageURL, resp.User.ProfileImageURL)
		assert.True(t, resp.User.HasProfileImage)
		assert.Equal(t, "mamadou@example.com", resp.User.Email)

		mockFileClient.AssertExpectations(t)
		mockAuthClient.AssertExpectations(t)
	})

	t.Run("erreur - image vide", func(t *testing.T) {
		cleanCollection(t)

		_, err := grpcClient.ChangeProfilePicture(ctxWithUID("firebase-photo-e2e"), &userpb.ChangeProfilePictureRequest{
			UserID:            "some-user-id",
	...
	t.Run("erreur - utilisateur non trouve", func(t *testing.T) {
		...
		mockFileClient.On("UploadProfilePicture", mock.Anything, "user-inexistant", mock.Anything).Return("", nil).Maybe()

		_, err := grpcClient.ChangeProfilePicture(ctxWithUID("firebase-photo-e2e"), &userpb.ChangeProfilePictureRequest{
			UserID:            "user-inexistant",
			NewProfilePicture: []byte{0xFF, 0xD8},
		})
	...
	t.Run("erreur - Firebase UID absent", func(t *testing.T) {
		...
		_, err := grpcClient.ChangeProfilePicture(context.Background(), &userpb.ChangeProfilePictureRequest{
			UserID:            "some-user-id",
			NewProfilePicture: []byte{0xFF, 0xD8},
		})
