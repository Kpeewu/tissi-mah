// File Service Proto Definition
// Gère l'upload de fichiers vers S3/MinIO et le stockage des métadonnées
// Certains endpoints sont exposés via api-gateway (HTTP), d'autres sont inter-service uniquement

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.35.1
// source: file.proto

package file

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Métadonnées légales d'un document véhicule
type VehicleDocMetadata struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	DocumentNumber   string                 `protobuf:"bytes,1,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"`
	IssuedAt         string                 `protobuf:"bytes,2,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"` // ISO 8601
	ExpireAt         string                 `protobuf:"bytes,3,opt,name=ExpireAt,proto3" json:"ExpireAt,omitempty"` // ISO 8601 — optionnel pour registrationCard
	IssuingAuthority string                 `protobuf:"bytes,4,opt,name=IssuingAuthority,proto3" json:"IssuingAuthority,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *VehicleDocMetadata) Reset() {
	*x = VehicleDocMetadata{}
	mi := &file_file_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *VehicleDocMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VehicleDocMetadata) ProtoMessage() {}

func (x *VehicleDocMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VehicleDocMetadata.ProtoReflect.Descriptor instead.
func (*VehicleDocMetadata) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{0}
}

func (x *VehicleDocMetadata) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *VehicleDocMetadata) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

func (x *VehicleDocMetadata) GetExpireAt() string {
	if x != nil {
		return x.ExpireAt
	}
	return ""
}

func (x *VehicleDocMetadata) GetIssuingAuthority() string {
	if x != nil {
		return x.IssuingAuthority
	}
	return ""
}

type UploadVehicleDocumentsRequest struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	UserID    string                 `protobuf:"bytes,1,opt,name=UserID,proto3" json:"UserID,omitempty"`
	VehicleID string                 `protobuf:"bytes,2,opt,name=VehicleID,proto3" json:"VehicleID,omitempty"`
	// Permis : optionnel si l'utilisateur a déjà un permis courant (soumis via
	// uploadIdDocument ou un précédent flux véhicule) — les images sont alors ignorées.
	// Obligatoire sinon (recto + verso). Stocké comme documents UTILISATEUR
	// (types driverLicenceFront/driverLicenceBack), jamais comme document véhicule :
	// il couvre tous les véhicules du user.
	DriverLicenceRecto       []byte              `protobuf:"bytes,9,opt,name=DriverLicenceRecto,proto3" json:"DriverLicenceRecto,omitempty"`
	DriverLicenceVerso       []byte              `protobuf:"bytes,10,opt,name=DriverLicenceVerso,proto3" json:"DriverLicenceVerso,omitempty"`
	Assurance                []byte              `protobuf:"bytes,4,opt,name=Assurance,proto3" json:"Assurance,omitempty"`
	VehicleRegistration      []byte              `protobuf:"bytes,5,opt,name=VehicleRegistration,proto3" json:"VehicleRegistration,omitempty"`
	DriverLicenceMetadata    *VehicleDocMetadata `protobuf:"bytes,6,opt,name=DriverLicenceMetadata,proto3" json:"DriverLicenceMetadata,omitempty"` // Requis uniquement si le permis est fourni et pris en compte (partagée recto/verso)
	AssuranceMetadata        *VehicleDocMetadata `protobuf:"bytes,7,opt,name=AssuranceMetadata,proto3" json:"AssuranceMetadata,omitempty"`
	RegistrationCardMetadata *VehicleDocMetadata `protobuf:"bytes,8,opt,name=RegistrationCardMetadata,proto3" json:"RegistrationCardMetadata,omitempty"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *UploadVehicleDocumentsRequest) Reset() {
	*x = UploadVehicleDocumentsRequest{}
	mi := &file_file_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadVehicleDocumentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadVehicleDocumentsRequest) ProtoMessage() {}

func (x *UploadVehicleDocumentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadVehicleDocumentsRequest.ProtoReflect.Descriptor instead.
func (*UploadVehicleDocumentsRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{1}
}

func (x *UploadVehicleDocumentsRequest) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

func (x *UploadVehicleDocumentsRequest) GetVehicleID() string {
	if x != nil {
		return x.VehicleID
	}
	return ""
}

func (x *UploadVehicleDocumentsRequest) GetDriverLicenceRecto() []byte {
	if x != nil {
		return x.DriverLicenceRecto
	}
	return nil
}

func (x *UploadVehicleDocumentsRequest) GetDriverLicenceVerso() []byte {
	if x != nil {
		return x.DriverLicenceVerso
	}
	return nil
}

func (x *UploadVehicleDocumentsRequest) GetAssurance() []byte {
	if x != nil {
		return x.Assurance
	}
	return nil
}

func (x *UploadVehicleDocumentsRequest) GetVehicleRegistration() []byte {
	if x != nil {
		return x.VehicleRegistration
	}
	return nil
}

func (x *UploadVehicleDocumentsRequest) GetDriverLicenceMetadata() *VehicleDocMetadata {
	if x != nil {
		return x.DriverLicenceMetadata
	}
	return nil
}

func (x *UploadVehicleDocumentsRequest) GetAssuranceMetadata() *VehicleDocMetadata {
	if x != nil {
		return x.AssuranceMetadata
	}
	return nil
}

func (x *UploadVehicleDocumentsRequest) GetRegistrationCardMetadata() *VehicleDocMetadata {
	if x != nil {
		return x.RegistrationCardMetadata
	}
	return nil
}

type UploadVehicleDocumentsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	ErrorMessage  string                 `protobuf:"bytes,2,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	Documents     []*UploadedDocument    `protobuf:"bytes,3,rep,name=Documents,proto3" json:"Documents,omitempty"` // [Permis recto + verso (si uploadé),] assurance, carte grise (dans cet ordre)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadVehicleDocumentsResponse) Reset() {
	*x = UploadVehicleDocumentsResponse{}
	mi := &file_file_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadVehicleDocumentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadVehicleDocumentsResponse) ProtoMessage() {}

func (x *UploadVehicleDocumentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadVehicleDocumentsResponse.ProtoReflect.Descriptor instead.
func (*UploadVehicleDocumentsResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{2}
}

func (x *UploadVehicleDocumentsResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *UploadVehicleDocumentsResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *UploadVehicleDocumentsResponse) GetDocuments() []*UploadedDocument {
	if x != nil {
		return x.Documents
	}
	return nil
}

// UploadedDocument décrit un document fraîchement uploadé/remplacé.
// Renvoyé par UploadIdDocument, UploadVehicleDocuments et ChangeDocument
// pour que le front affiche la ressource sans avoir à la re-fetcher.
type UploadedDocument struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	DocumentID          string                 `protobuf:"bytes,1,opt,name=DocumentID,proto3" json:"DocumentID,omitempty"`
	DocumentURL         string                 `protobuf:"bytes,2,opt,name=DocumentURL,proto3" json:"DocumentURL,omitempty"`
	DocumentType        string                 `protobuf:"bytes,3,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"`
	DocumentName        string                 `protobuf:"bytes,4,opt,name=DocumentName,proto3" json:"DocumentName,omitempty"`
	LogicalDocumentType string                 `protobuf:"bytes,5,opt,name=LogicalDocumentType,proto3" json:"LogicalDocumentType,omitempty"` // idCard, driverLicence, passport, insurance…
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *UploadedDocument) Reset() {
	*x = UploadedDocument{}
	mi := &file_file_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadedDocument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadedDocument) ProtoMessage() {}

func (x *UploadedDocument) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadedDocument.ProtoReflect.Descriptor instead.
func (*UploadedDocument) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{3}
}

func (x *UploadedDocument) GetDocumentID() string {
	if x != nil {
		return x.DocumentID
	}
	return ""
}

func (x *UploadedDocument) GetDocumentURL() string {
	if x != nil {
		return x.DocumentURL
	}
	return ""
}

func (x *UploadedDocument) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *UploadedDocument) GetDocumentName() string {
	if x != nil {
		return x.DocumentName
	}
	return ""
}

func (x *UploadedDocument) GetLogicalDocumentType() string {
	if x != nil {
		return x.LogicalDocumentType
	}
	return ""
}

type UploadSelfieRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Le propriétaire est résolu depuis x-firebase-uid (metadata gRPC injectée
	// par l'api-gateway) — aucun UserID accepté dans le body.
	Selfie        []byte `protobuf:"bytes,1,opt,name=Selfie,proto3" json:"Selfie,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadSelfieRequest) Reset() {
	*x = UploadSelfieRequest{}
	mi := &file_file_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadSelfieRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadSelfieRequest) ProtoMessage() {}

func (x *UploadSelfieRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadSelfieRequest.ProtoReflect.Descriptor instead.
func (*UploadSelfieRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{4}
}

func (x *UploadSelfieRequest) GetSelfie() []byte {
	if x != nil {
		return x.Selfie
	}
	return nil
}

type UploadSelfieResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	ErrorMessage  string                 `protobuf:"bytes,2,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	Document      *UploadedDocument      `protobuf:"bytes,3,opt,name=Document,proto3" json:"Document,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadSelfieResponse) Reset() {
	*x = UploadSelfieResponse{}
	mi := &file_file_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadSelfieResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadSelfieResponse) ProtoMessage() {}

func (x *UploadSelfieResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadSelfieResponse.ProtoReflect.Descriptor instead.
func (*UploadSelfieResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{5}
}

func (x *UploadSelfieResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *UploadSelfieResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *UploadSelfieResponse) GetDocument() *UploadedDocument {
	if x != nil {
		return x.Document
	}
	return nil
}

type DeleteFileRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserID        string                 `protobuf:"bytes,1,opt,name=UserID,proto3" json:"UserID,omitempty"`
	FileID        string                 `protobuf:"bytes,2,opt,name=FileID,proto3" json:"FileID,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteFileRequest) Reset() {
	*x = DeleteFileRequest{}
	mi := &file_file_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteFileRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteFileRequest) ProtoMessage() {}

func (x *DeleteFileRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteFileRequest.ProtoReflect.Descriptor instead.
func (*DeleteFileRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{6}
}

func (x *DeleteFileRequest) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

func (x *DeleteFileRequest) GetFileID() string {
	if x != nil {
		return x.FileID
	}
	return ""
}

type DeleteFileResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	ErrorMessage  string                 `protobuf:"bytes,2,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteFileResponse) Reset() {
	*x = DeleteFileResponse{}
	mi := &file_file_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteFileResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteFileResponse) ProtoMessage() {}

func (x *DeleteFileResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteFileResponse.ProtoReflect.Descriptor instead.
func (*DeleteFileResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{7}
}

func (x *DeleteFileResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *DeleteFileResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

type GetDocumentRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	FileID        string                 `protobuf:"bytes,1,opt,name=FileID,proto3" json:"FileID,omitempty"`
	UserID        string                 `protobuf:"bytes,2,opt,name=UserID,proto3" json:"UserID,omitempty"` // Optionnel — vérifie la propriété
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentRequest) Reset() {
	*x = GetDocumentRequest{}
	mi := &file_file_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentRequest) ProtoMessage() {}

func (x *GetDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{8}
}

func (x *GetDocumentRequest) GetFileID() string {
	if x != nil {
		return x.FileID
	}
	return ""
}

func (x *GetDocumentRequest) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

type DocumentFile struct {
	state                 protoimpl.MessageState `protogen:"open.v1"`
	FileID                string                 `protobuf:"bytes,1,opt,name=FileID,proto3" json:"FileID,omitempty"`
	FileURL               string                 `protobuf:"bytes,2,opt,name=FileURL,proto3" json:"FileURL,omitempty"` // URL présignée (1 h)
	FileType              string                 `protobuf:"bytes,3,opt,name=FileType,proto3" json:"FileType,omitempty"`
	PresignedUrlExpiresAt string                 `protobuf:"bytes,4,opt,name=PresignedUrlExpiresAt,proto3" json:"PresignedUrlExpiresAt,omitempty"` // ISO 8601
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}

func (x *DocumentFile) Reset() {
	*x = DocumentFile{}
	mi := &file_file_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentFile) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentFile) ProtoMessage() {}

func (x *DocumentFile) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentFile.ProtoReflect.Descriptor instead.
func (*DocumentFile) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{9}
}

func (x *DocumentFile) GetFileID() string {
	if x != nil {
		return x.FileID
	}
	return ""
}

func (x *DocumentFile) GetFileURL() string {
	if x != nil {
		return x.FileURL
	}
	return ""
}

func (x *DocumentFile) GetFileType() string {
	if x != nil {
		return x.FileType
	}
	return ""
}

func (x *DocumentFile) GetPresignedUrlExpiresAt() string {
	if x != nil {
		return x.PresignedUrlExpiresAt
	}
	return ""
}

type GetDocumentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ErrorMessage  string                 `protobuf:"bytes,1,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	File          *DocumentFile          `protobuf:"bytes,2,opt,name=File,proto3" json:"File,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentResponse) Reset() {
	*x = GetDocumentResponse{}
	mi := &file_file_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentResponse) ProtoMessage() {}

func (x *GetDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{10}
}

func (x *GetDocumentResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *GetDocumentResponse) GetFile() *DocumentFile {
	if x != nil {
		return x.File
	}
	return nil
}

type ChangeDocumentRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	UserID         string                 `protobuf:"bytes,1,opt,name=UserID,proto3" json:"UserID,omitempty"`
	FileID         string                 `protobuf:"bytes,2,opt,name=FileID,proto3" json:"FileID,omitempty"`
	NewDocument    []byte                 `protobuf:"bytes,3,opt,name=NewDocument,proto3" json:"NewDocument,omitempty"`
	DocumentNumber string                 `protobuf:"bytes,4,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"` // Optionnel — remplace l'existant si fourni
	IssuedAt       string                 `protobuf:"bytes,5,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"`             // Optionnel
	ExpireAt       string                 `protobuf:"bytes,6,opt,name=ExpireAt,proto3" json:"ExpireAt,omitempty"`             // Optionnel
	IssuingPlace   string                 `protobuf:"bytes,7,opt,name=IssuingPlace,proto3" json:"IssuingPlace,omitempty"`     // Optionnel (country pour user doc, authority pour vehicle doc)
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ChangeDocumentRequest) Reset() {
	*x = ChangeDocumentRequest{}
	mi := &file_file_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeDocumentRequest) ProtoMessage() {}

func (x *ChangeDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeDocumentRequest.ProtoReflect.Descriptor instead.
func (*ChangeDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{11}
}

func (x *ChangeDocumentRequest) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

func (x *ChangeDocumentRequest) GetFileID() string {
	if x != nil {
		return x.FileID
	}
	return ""
}

func (x *ChangeDocumentRequest) GetNewDocument() []byte {
	if x != nil {
		return x.NewDocument
	}
	return nil
}

func (x *ChangeDocumentRequest) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *ChangeDocumentRequest) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

func (x *ChangeDocumentRequest) GetExpireAt() string {
	if x != nil {
		return x.ExpireAt
	}
	return ""
}

func (x *ChangeDocumentRequest) GetIssuingPlace() string {
	if x != nil {
		return x.IssuingPlace
	}
	return ""
}

type ChangeDocumentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	ErrorMessage  string                 `protobuf:"bytes,2,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	Document      *UploadedDocument      `protobuf:"bytes,3,opt,name=Document,proto3" json:"Document,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeDocumentResponse) Reset() {
	*x = ChangeDocumentResponse{}
	mi := &file_file_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeDocumentResponse) ProtoMessage() {}

func (x *ChangeDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeDocumentResponse.ProtoReflect.Descriptor instead.
func (*ChangeDocumentResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{12}
}

func (x *ChangeDocumentResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *ChangeDocumentResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *ChangeDocumentResponse) GetDocument() *UploadedDocument {
	if x != nil {
		return x.Document
	}
	return nil
}

type UploadIdDocumentRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	UserID string                 `protobuf:"bytes,1,opt,name=UserID,proto3" json:"UserID,omitempty"`
	// Type : IDCard | Passport | DriverLicence
	DocumentType string `protobuf:"bytes,2,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"`
	// Obligatoires si DocumentType = IDCard
	IDCardRecto []byte `protobuf:"bytes,3,opt,name=IDCardRecto,proto3" json:"IDCardRecto,omitempty"`
	IDCardVerso []byte `protobuf:"bytes,4,opt,name=IDCardVerso,proto3" json:"IDCardVerso,omitempty"`
	// Obligatoires si DocumentType = DriverLicence (recto + verso).
	// Le permis est un document utilisateur partagé : validé/refusé pour
	// l'identité ET la vérification véhicule (tous les véhicules du user).
	DriverLicenceRecto []byte `protobuf:"bytes,12,opt,name=DriverLicenceRecto,proto3" json:"DriverLicenceRecto,omitempty"`
	DriverLicenceVerso []byte `protobuf:"bytes,13,opt,name=DriverLicenceVerso,proto3" json:"DriverLicenceVerso,omitempty"`
	// Obligatoire si DocumentType = Passport
	Passport []byte `protobuf:"bytes,7,opt,name=Passport,proto3" json:"Passport,omitempty"`
	// Métadonnées légales — obligatoires pour tous les types
	DocumentNumber string `protobuf:"bytes,8,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"`
	IssuedAt       string `protobuf:"bytes,9,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"`  // ISO 8601
	ExpireAt       string `protobuf:"bytes,10,opt,name=ExpireAt,proto3" json:"ExpireAt,omitempty"` // ISO 8601
	IssuingCountry string `protobuf:"bytes,11,opt,name=IssuingCountry,proto3" json:"IssuingCountry,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UploadIdDocumentRequest) Reset() {
	*x = UploadIdDocumentRequest{}
	mi := &file_file_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadIdDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadIdDocumentRequest) ProtoMessage() {}

func (x *UploadIdDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadIdDocumentRequest.ProtoReflect.Descriptor instead.
func (*UploadIdDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{13}
}

func (x *UploadIdDocumentRequest) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

func (x *UploadIdDocumentRequest) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *UploadIdDocumentRequest) GetIDCardRecto() []byte {
	if x != nil {
		return x.IDCardRecto
	}
	return nil
}

func (x *UploadIdDocumentRequest) GetIDCardVerso() []byte {
	if x != nil {
		return x.IDCardVerso
	}
	return nil
}

func (x *UploadIdDocumentRequest) GetDriverLicenceRecto() []byte {
	if x != nil {
		return x.DriverLicenceRecto
	}
	return nil
}

func (x *UploadIdDocumentRequest) GetDriverLicenceVerso() []byte {
	if x != nil {
		return x.DriverLicenceVerso
	}
	return nil
}

func (x *UploadIdDocumentRequest) GetPassport() []byte {
	if x != nil {
		return x.Passport
	}
	return nil
}

func (x *UploadIdDocumentRequest) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *UploadIdDocumentRequest) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

func (x *UploadIdDocumentRequest) GetExpireAt() string {
	if x != nil {
		return x.ExpireAt
	}
	return ""
}

func (x *UploadIdDocumentRequest) GetIssuingCountry() string {
	if x != nil {
		return x.IssuingCountry
	}
	return ""
}

type UploadIdDocumentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	ErrorMessage  string                 `protobuf:"bytes,2,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	Documents     []*UploadedDocument    `protobuf:"bytes,3,rep,name=Documents,proto3" json:"Documents,omitempty"` // 1 doc pour Passport, 2 pour IDCard / DriverLicence (recto + verso)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadIdDocumentResponse) Reset() {
	*x = UploadIdDocumentResponse{}
	mi := &file_file_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadIdDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadIdDocumentResponse) ProtoMessage() {}

func (x *UploadIdDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadIdDocumentResponse.ProtoReflect.Descriptor instead.
func (*UploadIdDocumentResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{14}
}

func (x *UploadIdDocumentResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *UploadIdDocumentResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *UploadIdDocumentResponse) GetDocuments() []*UploadedDocument {
	if x != nil {
		return x.Documents
	}
	return nil
}

type UploadUserDocumentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Data:
	//
	//	*UploadUserDocumentRequest_Metadata
	//	*UploadUserDocumentRequest_Chunk
	Data          isUploadUserDocumentRequest_Data `protobuf_oneof:"data"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadUserDocumentRequest) Reset() {
	*x = UploadUserDocumentRequest{}
	mi := &file_file_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadUserDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadUserDocumentRequest) ProtoMessage() {}

func (x *UploadUserDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadUserDocumentRequest.ProtoReflect.Descriptor instead.
func (*UploadUserDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{15}
}

func (x *UploadUserDocumentRequest) GetData() isUploadUserDocumentRequest_Data {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *UploadUserDocumentRequest) GetMetadata() *UserDocumentMetadata {
	if x != nil {
		if x, ok := x.Data.(*UploadUserDocumentRequest_Metadata); ok {
			return x.Metadata
		}
	}
	return nil
}

func (x *UploadUserDocumentRequest) GetChunk() []byte {
	if x != nil {
		if x, ok := x.Data.(*UploadUserDocumentRequest_Chunk); ok {
			return x.Chunk
		}
	}
	return nil
}

type isUploadUserDocumentRequest_Data interface {
	isUploadUserDocumentRequest_Data()
}

type UploadUserDocumentRequest_Metadata struct {
	Metadata *UserDocumentMetadata `protobuf:"bytes,1,opt,name=metadata,proto3,oneof"`
}

type UploadUserDocumentRequest_Chunk struct {
	Chunk []byte `protobuf:"bytes,2,opt,name=Chunk,proto3,oneof"`
}

func (*UploadUserDocumentRequest_Metadata) isUploadUserDocumentRequest_Data() {}

func (*UploadUserDocumentRequest_Chunk) isUploadUserDocumentRequest_Data() {}

type UserDocumentMetadata struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	UserId         string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`
	DocumentName   string                 `protobuf:"bytes,2,opt,name=DocumentName,proto3" json:"DocumentName,omitempty"`
	DocumentType   string                 `protobuf:"bytes,3,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"` // idCardFront, idCardBack, passport, etc.
	MimeType       string                 `protobuf:"bytes,4,opt,name=MimeType,proto3" json:"MimeType,omitempty"`
	FileSizeBytes  int64                  `protobuf:"varint,5,opt,name=FileSizeBytes,proto3" json:"FileSizeBytes,omitempty"`
	DocumentNumber string                 `protobuf:"bytes,6,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"` // Optionnel
	IssuingCountry string                 `protobuf:"bytes,7,opt,name=IssuingCountry,proto3" json:"IssuingCountry,omitempty"` // Optionnel
	IssuedAt       string                 `protobuf:"bytes,8,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"`             // ISO 8601 — optionnel pour profilePicture, obligatoire sinon
	ExpireAt       string                 `protobuf:"bytes,9,opt,name=ExpireAt,proto3" json:"ExpireAt,omitempty"`             // ISO 8601 — optionnel pour profilePicture, obligatoire sinon
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UserDocumentMetadata) Reset() {
	*x = UserDocumentMetadata{}
	mi := &file_file_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserDocumentMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserDocumentMetadata) ProtoMessage() {}

func (x *UserDocumentMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserDocumentMetadata.ProtoReflect.Descriptor instead.
func (*UserDocumentMetadata) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{16}
}

func (x *UserDocumentMetadata) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *UserDocumentMetadata) GetDocumentName() string {
	if x != nil {
		return x.DocumentName
	}
	return ""
}

func (x *UserDocumentMetadata) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *UserDocumentMetadata) GetMimeType() string {
	if x != nil {
		return x.MimeType
	}
	return ""
}

func (x *UserDocumentMetadata) GetFileSizeBytes() int64 {
	if x != nil {
		return x.FileSizeBytes
	}
	return 0
}

func (x *UserDocumentMetadata) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *UserDocumentMetadata) GetIssuingCountry() string {
	if x != nil {
		return x.IssuingCountry
	}
	return ""
}

func (x *UserDocumentMetadata) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

func (x *UserDocumentMetadata) GetExpireAt() string {
	if x != nil {
		return x.ExpireAt
	}
	return ""
}

type UploadVehicleDocumentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Data:
	//
	//	*UploadVehicleDocumentRequest_Metadata
	//	*UploadVehicleDocumentRequest_Chunk
	Data          isUploadVehicleDocumentRequest_Data `protobuf_oneof:"data"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadVehicleDocumentRequest) Reset() {
	*x = UploadVehicleDocumentRequest{}
	mi := &file_file_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadVehicleDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadVehicleDocumentRequest) ProtoMessage() {}

func (x *UploadVehicleDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadVehicleDocumentRequest.ProtoReflect.Descriptor instead.
func (*UploadVehicleDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{17}
}

func (x *UploadVehicleDocumentRequest) GetData() isUploadVehicleDocumentRequest_Data {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *UploadVehicleDocumentRequest) GetMetadata() *VehicleDocumentMetadata {
	if x != nil {
		if x, ok := x.Data.(*UploadVehicleDocumentRequest_Metadata); ok {
			return x.Metadata
		}
	}
	return nil
}

func (x *UploadVehicleDocumentRequest) GetChunk() []byte {
	if x != nil {
		if x, ok := x.Data.(*UploadVehicleDocumentRequest_Chunk); ok {
			return x.Chunk
		}
	}
	return nil
}

type isUploadVehicleDocumentRequest_Data interface {
	isUploadVehicleDocumentRequest_Data()
}

type UploadVehicleDocumentRequest_Metadata struct {
	Metadata *VehicleDocumentMetadata `protobuf:"bytes,1,opt,name=metadata,proto3,oneof"`
}

type UploadVehicleDocumentRequest_Chunk struct {
	Chunk []byte `protobuf:"bytes,2,opt,name=Chunk,proto3,oneof"`
}

func (*UploadVehicleDocumentRequest_Metadata) isUploadVehicleDocumentRequest_Data() {}

func (*UploadVehicleDocumentRequest_Chunk) isUploadVehicleDocumentRequest_Data() {}

type VehicleDocumentMetadata struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	VehicleId        string                 `protobuf:"bytes,1,opt,name=VehicleId,proto3" json:"VehicleId,omitempty"`
	DocumentName     string                 `protobuf:"bytes,2,opt,name=DocumentName,proto3" json:"DocumentName,omitempty"`
	DocumentType     string                 `protobuf:"bytes,3,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"` // insurance, registrationCard
	MimeType         string                 `protobuf:"bytes,4,opt,name=MimeType,proto3" json:"MimeType,omitempty"`
	FileSizeBytes    int64                  `protobuf:"varint,5,opt,name=FileSizeBytes,proto3" json:"FileSizeBytes,omitempty"`
	DocumentNumber   string                 `protobuf:"bytes,6,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"`     // Optionnel
	IssuingAuthority string                 `protobuf:"bytes,7,opt,name=IssuingAuthority,proto3" json:"IssuingAuthority,omitempty"` // Optionnel
	IssuedAt         string                 `protobuf:"bytes,8,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"`                 // ISO 8601 — obligatoire pour insurance/registrationCard
	ExpireAt         string                 `protobuf:"bytes,9,opt,name=ExpireAt,proto3" json:"ExpireAt,omitempty"`                 // ISO 8601 — obligatoire pour insurance, optionnel pour registrationCard
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *VehicleDocumentMetadata) Reset() {
	*x = VehicleDocumentMetadata{}
	mi := &file_file_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *VehicleDocumentMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VehicleDocumentMetadata) ProtoMessage() {}

func (x *VehicleDocumentMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VehicleDocumentMetadata.ProtoReflect.Descriptor instead.
func (*VehicleDocumentMetadata) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{18}
}

func (x *VehicleDocumentMetadata) GetVehicleId() string {
	if x != nil {
		return x.VehicleId
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetDocumentName() string {
	if x != nil {
		return x.DocumentName
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetMimeType() string {
	if x != nil {
		return x.MimeType
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetFileSizeBytes() int64 {
	if x != nil {
		return x.FileSizeBytes
	}
	return 0
}

func (x *VehicleDocumentMetadata) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetIssuingAuthority() string {
	if x != nil {
		return x.IssuingAuthority
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

func (x *VehicleDocumentMetadata) GetExpireAt() string {
	if x != nil {
		return x.ExpireAt
	}
	return ""
}

type GetUserDocumentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserDocumentsRequest) Reset() {
	*x = GetUserDocumentsRequest{}
	mi := &file_file_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserDocumentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserDocumentsRequest) ProtoMessage() {}

func (x *GetUserDocumentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserDocumentsRequest.ProtoReflect.Descriptor instead.
func (*GetUserDocumentsRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{19}
}

func (x *GetUserDocumentsRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

type GetDocumentByIDRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	DocumentId     string                 `protobuf:"bytes,1,opt,name=DocumentId,proto3" json:"DocumentId,omitempty"`
	PresignTTLSecs int64                  `protobuf:"varint,2,opt,name=PresignTTLSecs,proto3" json:"PresignTTLSecs,omitempty"` // 0 = défaut 1 h
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetDocumentByIDRequest) Reset() {
	*x = GetDocumentByIDRequest{}
	mi := &file_file_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentByIDRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentByIDRequest) ProtoMessage() {}

func (x *GetDocumentByIDRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentByIDRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentByIDRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{20}
}

func (x *GetDocumentByIDRequest) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

func (x *GetDocumentByIDRequest) GetPresignTTLSecs() int64 {
	if x != nil {
		return x.PresignTTLSecs
	}
	return 0
}

type GetCurrentUserDocumentRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`
	DocumentType  string                 `protobuf:"bytes,2,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetCurrentUserDocumentRequest) Reset() {
	*x = GetCurrentUserDocumentRequest{}
	mi := &file_file_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetCurrentUserDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCurrentUserDocumentRequest) ProtoMessage() {}

func (x *GetCurrentUserDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCurrentUserDocumentRequest.ProtoReflect.Descriptor instead.
func (*GetCurrentUserDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{21}
}

func (x *GetCurrentUserDocumentRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *GetCurrentUserDocumentRequest) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

type GetVehicleDocumentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	VehicleId     string                 `protobuf:"bytes,1,opt,name=VehicleId,proto3" json:"VehicleId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetVehicleDocumentsRequest) Reset() {
	*x = GetVehicleDocumentsRequest{}
	mi := &file_file_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetVehicleDocumentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetVehicleDocumentsRequest) ProtoMessage() {}

func (x *GetVehicleDocumentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetVehicleDocumentsRequest.ProtoReflect.Descriptor instead.
func (*GetVehicleDocumentsRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{22}
}

func (x *GetVehicleDocumentsRequest) GetVehicleId() string {
	if x != nil {
		return x.VehicleId
	}
	return ""
}

type GetVehicleDocumentsByUserIDRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetVehicleDocumentsByUserIDRequest) Reset() {
	*x = GetVehicleDocumentsByUserIDRequest{}
	mi := &file_file_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetVehicleDocumentsByUserIDRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetVehicleDocumentsByUserIDRequest) ProtoMessage() {}

func (x *GetVehicleDocumentsByUserIDRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetVehicleDocumentsByUserIDRequest.ProtoReflect.Descriptor instead.
func (*GetVehicleDocumentsByUserIDRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{23}
}

func (x *GetVehicleDocumentsByUserIDRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

type ListKycDocumentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Statuses      []string               `protobuf:"bytes,1,rep,name=Statuses,proto3" json:"Statuses,omitempty"` // vide = tous les statuts
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListKycDocumentsRequest) Reset() {
	*x = ListKycDocumentsRequest{}
	mi := &file_file_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListKycDocumentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListKycDocumentsRequest) ProtoMessage() {}

func (x *ListKycDocumentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListKycDocumentsRequest.ProtoReflect.Descriptor instead.
func (*ListKycDocumentsRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{24}
}

func (x *ListKycDocumentsRequest) GetStatuses() []string {
	if x != nil {
		return x.Statuses
	}
	return nil
}

// KycDocument : vue unifiée d'un document KYC (user ou véhicule) pour la file
// de validation manuelle. OwnerKind = "user" | "vehicle".
type KycDocument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DocumentId    string                 `protobuf:"bytes,1,opt,name=DocumentId,proto3" json:"DocumentId,omitempty"`
	UserId        string                 `protobuf:"bytes,2,opt,name=UserId,proto3" json:"UserId,omitempty"`
	VehicleId     string                 `protobuf:"bytes,3,opt,name=VehicleId,proto3" json:"VehicleId,omitempty"` // vide pour un document utilisateur
	DocumentType  string                 `protobuf:"bytes,4,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"`
	Status        string                 `protobuf:"bytes,5,opt,name=Status,proto3" json:"Status,omitempty"`
	OwnerKind     string                 `protobuf:"bytes,6,opt,name=OwnerKind,proto3" json:"OwnerKind,omitempty"`
	UpdatedAt     string                 `protobuf:"bytes,7,opt,name=UpdatedAt,proto3" json:"UpdatedAt,omitempty"`   // ISO 8601
	UploadedAt    string                 `protobuf:"bytes,8,opt,name=UploadedAt,proto3" json:"UploadedAt,omitempty"` // ISO 8601 — date de dépôt initial
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *KycDocument) Reset() {
	*x = KycDocument{}
	mi := &file_file_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KycDocument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KycDocument) ProtoMessage() {}

func (x *KycDocument) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KycDocument.ProtoReflect.Descriptor instead.
func (*KycDocument) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{25}
}

func (x *KycDocument) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

func (x *KycDocument) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *KycDocument) GetVehicleId() string {
	if x != nil {
		return x.VehicleId
	}
	return ""
}

func (x *KycDocument) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *KycDocument) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *KycDocument) GetOwnerKind() string {
	if x != nil {
		return x.OwnerKind
	}
	return ""
}

func (x *KycDocument) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *KycDocument) GetUploadedAt() string {
	if x != nil {
		return x.UploadedAt
	}
	return ""
}

type ListKycDocumentsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Documents     []*KycDocument         `protobuf:"bytes,1,rep,name=Documents,proto3" json:"Documents,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListKycDocumentsResponse) Reset() {
	*x = ListKycDocumentsResponse{}
	mi := &file_file_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListKycDocumentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListKycDocumentsResponse) ProtoMessage() {}

func (x *ListKycDocumentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListKycDocumentsResponse.ProtoReflect.Descriptor instead.
func (*ListKycDocumentsResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{26}
}

func (x *ListKycDocumentsResponse) GetDocuments() []*KycDocument {
	if x != nil {
		return x.Documents
	}
	return nil
}

type DeleteDocumentRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DocumentId    string                 `protobuf:"bytes,1,opt,name=DocumentId,proto3" json:"DocumentId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteDocumentRequest) Reset() {
	*x = DeleteDocumentRequest{}
	mi := &file_file_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteDocumentRequest) ProtoMessage() {}

func (x *DeleteDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteDocumentRequest.ProtoReflect.Descriptor instead.
func (*DeleteDocumentRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{27}
}

func (x *DeleteDocumentRequest) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

type CreateDocumentReviewRequest struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	UserDocumentId    string                 `protobuf:"bytes,1,opt,name=UserDocumentId,proto3" json:"UserDocumentId,omitempty"`       // Optionnel
	VehicleDocumentId string                 `protobuf:"bytes,2,opt,name=VehicleDocumentId,proto3" json:"VehicleDocumentId,omitempty"` // Optionnel
	// Retry / versioning
	AttemptNumber    int32  `protobuf:"varint,10,opt,name=AttemptNumber,proto3" json:"AttemptNumber,omitempty"`      // Défaut 1
	PreviousReviewId string `protobuf:"bytes,11,opt,name=PreviousReviewId,proto3" json:"PreviousReviewId,omitempty"` // Optionnel
	// Décision
	Status           string `protobuf:"bytes,12,opt,name=Status,proto3" json:"Status,omitempty"`                     // pending | completed | expired | failed
	Decision         string `protobuf:"bytes,13,opt,name=Decision,proto3" json:"Decision,omitempty"`                 // approved, rejected, resubmission
	ReasonRejection  string `protobuf:"bytes,14,opt,name=ReasonRejection,proto3" json:"ReasonRejection,omitempty"`   // Optionnel (enum)
	RejectionDetails string `protobuf:"bytes,15,opt,name=RejectionDetails,proto3" json:"RejectionDetails,omitempty"` // Optionnel
	// Réviseur
	ReviewedBy    string `protobuf:"bytes,16,opt,name=ReviewedBy,proto3" json:"ReviewedBy,omitempty"`
	ReviewType    string `protobuf:"bytes,17,opt,name=ReviewType,proto3" json:"ReviewType,omitempty"`       // manual
	Notes         string `protobuf:"bytes,18,opt,name=Notes,proto3" json:"Notes,omitempty"`                 // Optionnel
	ExtractedData []byte `protobuf:"bytes,19,opt,name=ExtractedData,proto3" json:"ExtractedData,omitempty"` // Optionnel (JSON)
	// Timestamps
	SubmittedAt string `protobuf:"bytes,20,opt,name=SubmittedAt,proto3" json:"SubmittedAt,omitempty"` // Optionnel (ISO 8601)
	// Dénormalisation (migration 000008) — toujours fournis par kyc-service
	UserId       string `protobuf:"bytes,21,opt,name=UserId,proto3" json:"UserId,omitempty"`             // Obligatoire — propriétaire de la review
	DocumentType string `protobuf:"bytes,22,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"` // Type de document (idCardFront, driverLicenceFront, ...)
	// Recto-verso + historique (migration 000010)
	SecondUserDocumentId string `protobuf:"bytes,23,opt,name=SecondUserDocumentId,proto3" json:"SecondUserDocumentId,omitempty"` // Optionnel — verso pour les documents recto-verso
	LogicalDocumentType  string `protobuf:"bytes,24,opt,name=LogicalDocumentType,proto3" json:"LogicalDocumentType,omitempty"`   // idCard, driverLicence, passport… (dérivé de DocumentType)
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *CreateDocumentReviewRequest) Reset() {
	*x = CreateDocumentReviewRequest{}
	mi := &file_file_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateDocumentReviewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateDocumentReviewRequest) ProtoMessage() {}

func (x *CreateDocumentReviewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateDocumentReviewRequest.ProtoReflect.Descriptor instead.
func (*CreateDocumentReviewRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{28}
}

func (x *CreateDocumentReviewRequest) GetUserDocumentId() string {
	if x != nil {
		return x.UserDocumentId
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetVehicleDocumentId() string {
	if x != nil {
		return x.VehicleDocumentId
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetAttemptNumber() int32 {
	if x != nil {
		return x.AttemptNumber
	}
	return 0
}

func (x *CreateDocumentReviewRequest) GetPreviousReviewId() string {
	if x != nil {
		return x.PreviousReviewId
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetDecision() string {
	if x != nil {
		return x.Decision
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetReasonRejection() string {
	if x != nil {
		return x.ReasonRejection
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetRejectionDetails() string {
	if x != nil {
		return x.RejectionDetails
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetReviewedBy() string {
	if x != nil {
		return x.ReviewedBy
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetReviewType() string {
	if x != nil {
		return x.ReviewType
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetNotes() string {
	if x != nil {
		return x.Notes
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetExtractedData() []byte {
	if x != nil {
		return x.ExtractedData
	}
	return nil
}

func (x *CreateDocumentReviewRequest) GetSubmittedAt() string {
	if x != nil {
		return x.SubmittedAt
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetSecondUserDocumentId() string {
	if x != nil {
		return x.SecondUserDocumentId
	}
	return ""
}

func (x *CreateDocumentReviewRequest) GetLogicalDocumentType() string {
	if x != nil {
		return x.LogicalDocumentType
	}
	return ""
}

type GetDocumentReviewsRequest struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	UserDocumentId    string                 `protobuf:"bytes,1,opt,name=UserDocumentId,proto3" json:"UserDocumentId,omitempty"`       // Optionnel
	VehicleDocumentId string                 `protobuf:"bytes,2,opt,name=VehicleDocumentId,proto3" json:"VehicleDocumentId,omitempty"` // Optionnel
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *GetDocumentReviewsRequest) Reset() {
	*x = GetDocumentReviewsRequest{}
	mi := &file_file_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentReviewsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentReviewsRequest) ProtoMessage() {}

func (x *GetDocumentReviewsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentReviewsRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentReviewsRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{29}
}

func (x *GetDocumentReviewsRequest) GetUserDocumentId() string {
	if x != nil {
		return x.UserDocumentId
	}
	return ""
}

func (x *GetDocumentReviewsRequest) GetVehicleDocumentId() string {
	if x != nil {
		return x.VehicleDocumentId
	}
	return ""
}

type GetDocumentReviewByIDRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ReviewId      string                 `protobuf:"bytes,1,opt,name=ReviewId,proto3" json:"ReviewId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentReviewByIDRequest) Reset() {
	*x = GetDocumentReviewByIDRequest{}
	mi := &file_file_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentReviewByIDRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentReviewByIDRequest) ProtoMessage() {}

func (x *GetDocumentReviewByIDRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentReviewByIDRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentReviewByIDRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{30}
}

func (x *GetDocumentReviewByIDRequest) GetReviewId() string {
	if x != nil {
		return x.ReviewId
	}
	return ""
}

type GetDocumentReviewsByUserIDRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentReviewsByUserIDRequest) Reset() {
	*x = GetDocumentReviewsByUserIDRequest{}
	mi := &file_file_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentReviewsByUserIDRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentReviewsByUserIDRequest) ProtoMessage() {}

func (x *GetDocumentReviewsByUserIDRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentReviewsByUserIDRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentReviewsByUserIDRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{31}
}

func (x *GetDocumentReviewsByUserIDRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

type UpdateDocumentReviewRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	ReviewId string                 `protobuf:"bytes,1,opt,name=ReviewId,proto3" json:"ReviewId,omitempty"`
	// Décision
	Status           string `protobuf:"bytes,7,opt,name=Status,proto3" json:"Status,omitempty"`                      // Optionnel
	Decision         string `protobuf:"bytes,8,opt,name=Decision,proto3" json:"Decision,omitempty"`                  // Optionnel
	ReasonRejection  string `protobuf:"bytes,9,opt,name=ReasonRejection,proto3" json:"ReasonRejection,omitempty"`    // Optionnel
	RejectionDetails string `protobuf:"bytes,10,opt,name=RejectionDetails,proto3" json:"RejectionDetails,omitempty"` // Optionnel
	// Réviseur
	ReviewedBy string `protobuf:"bytes,11,opt,name=ReviewedBy,proto3" json:"ReviewedBy,omitempty"` // Optionnel
	ReviewType string `protobuf:"bytes,12,opt,name=ReviewType,proto3" json:"ReviewType,omitempty"` // Optionnel
	Notes      string `protobuf:"bytes,13,opt,name=Notes,proto3" json:"Notes,omitempty"`           // Optionnel
	// Timestamps
	SubmittedAt string `protobuf:"bytes,14,opt,name=SubmittedAt,proto3" json:"SubmittedAt,omitempty"` // Optionnel (ISO 8601)
	// Recto-verso : verso résolu par kyc-service au moment de la décision.
	// Si vide pour un type recto-verso, le file-service résout la face compagnon
	// courante lui-même avant de synchroniser les statuts.
	SecondUserDocumentId string `protobuf:"bytes,15,opt,name=SecondUserDocumentId,proto3" json:"SecondUserDocumentId,omitempty"` // Optionnel
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *UpdateDocumentReviewRequest) Reset() {
	*x = UpdateDocumentReviewRequest{}
	mi := &file_file_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateDocumentReviewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateDocumentReviewRequest) ProtoMessage() {}

func (x *UpdateDocumentReviewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateDocumentReviewRequest.ProtoReflect.Descriptor instead.
func (*UpdateDocumentReviewRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{32}
}

func (x *UpdateDocumentReviewRequest) GetReviewId() string {
	if x != nil {
		return x.ReviewId
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetDecision() string {
	if x != nil {
		return x.Decision
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetReasonRejection() string {
	if x != nil {
		return x.ReasonRejection
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetRejectionDetails() string {
	if x != nil {
		return x.RejectionDetails
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetReviewedBy() string {
	if x != nil {
		return x.ReviewedBy
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetReviewType() string {
	if x != nil {
		return x.ReviewType
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetNotes() string {
	if x != nil {
		return x.Notes
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetSubmittedAt() string {
	if x != nil {
		return x.SubmittedAt
	}
	return ""
}

func (x *UpdateDocumentReviewRequest) GetSecondUserDocumentId() string {
	if x != nil {
		return x.SecondUserDocumentId
	}
	return ""
}

type ListDocumentReviewsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`      // Optionnel — filtre par utilisateur
	Status        string                 `protobuf:"bytes,2,opt,name=Status,proto3" json:"Status,omitempty"`      // Optionnel — filtre par statut
	Decision      string                 `protobuf:"bytes,3,opt,name=Decision,proto3" json:"Decision,omitempty"`  // Optionnel — filtre par décision
	Page          int32                  `protobuf:"varint,4,opt,name=Page,proto3" json:"Page,omitempty"`         // Numéro de page (0-based)
	PageSize      int32                  `protobuf:"varint,5,opt,name=PageSize,proto3" json:"PageSize,omitempty"` // Nombre de résultats par page
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDocumentReviewsRequest) Reset() {
	*x = ListDocumentReviewsRequest{}
	mi := &file_file_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDocumentReviewsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDocumentReviewsRequest) ProtoMessage() {}

func (x *ListDocumentReviewsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDocumentReviewsRequest.ProtoReflect.Descriptor instead.
func (*ListDocumentReviewsRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{33}
}

func (x *ListDocumentReviewsRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *ListDocumentReviewsRequest) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *ListDocumentReviewsRequest) GetDecision() string {
	if x != nil {
		return x.Decision
	}
	return ""
}

func (x *ListDocumentReviewsRequest) GetPage() int32 {
	if x != nil {
		return x.Page
	}
	return 0
}

func (x *ListDocumentReviewsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

type HealthRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HealthRequest) Reset() {
	*x = HealthRequest{}
	mi := &file_file_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HealthRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HealthRequest) ProtoMessage() {}

func (x *HealthRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HealthRequest.ProtoReflect.Descriptor instead.
func (*HealthRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{34}
}

type UserDocumentResponse struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	DocumentId          string                 `protobuf:"bytes,1,opt,name=DocumentId,proto3" json:"DocumentId,omitempty"`
	UserId              string                 `protobuf:"bytes,2,opt,name=UserId,proto3" json:"UserId,omitempty"`
	DocumentName        string                 `protobuf:"bytes,3,opt,name=DocumentName,proto3" json:"DocumentName,omitempty"`
	DocumentType        string                 `protobuf:"bytes,4,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"`
	DocumentUrl         string                 `protobuf:"bytes,5,opt,name=DocumentUrl,proto3" json:"DocumentUrl,omitempty"`
	FileSizeBytes       int64                  `protobuf:"varint,6,opt,name=FileSizeBytes,proto3" json:"FileSizeBytes,omitempty"`
	MimeType            string                 `protobuf:"bytes,7,opt,name=MimeType,proto3" json:"MimeType,omitempty"`
	DocumentNumber      string                 `protobuf:"bytes,8,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"`
	IssuingCountry      string                 `protobuf:"bytes,9,opt,name=IssuingCountry,proto3" json:"IssuingCountry,omitempty"`
	Status              string                 `protobuf:"bytes,10,opt,name=Status,proto3" json:"Status,omitempty"`
	IsCurrent           bool                   `protobuf:"varint,11,opt,name=IsCurrent,proto3" json:"IsCurrent,omitempty"`
	UploadedAt          string                 `protobuf:"bytes,12,opt,name=UploadedAt,proto3" json:"UploadedAt,omitempty"`
	UpdatedAt           string                 `protobuf:"bytes,13,opt,name=UpdatedAt,proto3" json:"UpdatedAt,omitempty"`
	ExpiredAt           string                 `protobuf:"bytes,14,opt,name=ExpiredAt,proto3" json:"ExpiredAt,omitempty"`                     // ISO 8601 ou "" si non défini
	LogicalDocumentType string                 `protobuf:"bytes,15,opt,name=LogicalDocumentType,proto3" json:"LogicalDocumentType,omitempty"` // idCard, driverLicence, passport…
	IssuedAt            string                 `protobuf:"bytes,16,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"`                       // ISO 8601 ou "" si non défini
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *UserDocumentResponse) Reset() {
	*x = UserDocumentResponse{}
	mi := &file_file_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserDocumentResponse) ProtoMessage() {}

func (x *UserDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserDocumentResponse.ProtoReflect.Descriptor instead.
func (*UserDocumentResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{35}
}

func (x *UserDocumentResponse) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

func (x *UserDocumentResponse) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *UserDocumentResponse) GetDocumentName() string {
	if x != nil {
		return x.DocumentName
	}
	return ""
}

func (x *UserDocumentResponse) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *UserDocumentResponse) GetDocumentUrl() string {
	if x != nil {
		return x.DocumentUrl
	}
	return ""
}

func (x *UserDocumentResponse) GetFileSizeBytes() int64 {
	if x != nil {
		return x.FileSizeBytes
	}
	return 0
}

func (x *UserDocumentResponse) GetMimeType() string {
	if x != nil {
		return x.MimeType
	}
	return ""
}

func (x *UserDocumentResponse) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *UserDocumentResponse) GetIssuingCountry() string {
	if x != nil {
		return x.IssuingCountry
	}
	return ""
}

func (x *UserDocumentResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *UserDocumentResponse) GetIsCurrent() bool {
	if x != nil {
		return x.IsCurrent
	}
	return false
}

func (x *UserDocumentResponse) GetUploadedAt() string {
	if x != nil {
		return x.UploadedAt
	}
	return ""
}

func (x *UserDocumentResponse) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *UserDocumentResponse) GetExpiredAt() string {
	if x != nil {
		return x.ExpiredAt
	}
	return ""
}

func (x *UserDocumentResponse) GetLogicalDocumentType() string {
	if x != nil {
		return x.LogicalDocumentType
	}
	return ""
}

func (x *UserDocumentResponse) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

// VehicleInfo embarque les données essentielles du véhicule dans la réponse document.
type VehicleInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	VehicleId     string                 `protobuf:"bytes,1,opt,name=VehicleId,proto3" json:"VehicleId,omitempty"`
	Brand         string                 `protobuf:"bytes,2,opt,name=Brand,proto3" json:"Brand,omitempty"`
	BrandModel    string                 `protobuf:"bytes,3,opt,name=BrandModel,proto3" json:"BrandModel,omitempty"`
	Color         string                 `protobuf:"bytes,4,opt,name=Color,proto3" json:"Color,omitempty"`
	LicencePlate  string                 `protobuf:"bytes,5,opt,name=LicencePlate,proto3" json:"LicencePlate,omitempty"`
	NumberOfSeats int32                  `protobuf:"varint,6,opt,name=NumberOfSeats,proto3" json:"NumberOfSeats,omitempty"`
	IsVerified    bool                   `protobuf:"varint,7,opt,name=IsVerified,proto3" json:"IsVerified,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *VehicleInfo) Reset() {
	*x = VehicleInfo{}
	mi := &file_file_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *VehicleInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VehicleInfo) ProtoMessage() {}

func (x *VehicleInfo) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VehicleInfo.ProtoReflect.Descriptor instead.
func (*VehicleInfo) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{36}
}

func (x *VehicleInfo) GetVehicleId() string {
	if x != nil {
		return x.VehicleId
	}
	return ""
}

func (x *VehicleInfo) GetBrand() string {
	if x != nil {
		return x.Brand
	}
	return ""
}

func (x *VehicleInfo) GetBrandModel() string {
	if x != nil {
		return x.BrandModel
	}
	return ""
}

func (x *VehicleInfo) GetColor() string {
	if x != nil {
		return x.Color
	}
	return ""
}

func (x *VehicleInfo) GetLicencePlate() string {
	if x != nil {
		return x.LicencePlate
	}
	return ""
}

func (x *VehicleInfo) GetNumberOfSeats() int32 {
	if x != nil {
		return x.NumberOfSeats
	}
	return 0
}

func (x *VehicleInfo) GetIsVerified() bool {
	if x != nil {
		return x.IsVerified
	}
	return false
}

type VehicleDocumentResponse struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	DocumentId       string                 `protobuf:"bytes,1,opt,name=DocumentId,proto3" json:"DocumentId,omitempty"`
	VehicleId        string                 `protobuf:"bytes,2,opt,name=VehicleId,proto3" json:"VehicleId,omitempty"`
	DocumentName     string                 `protobuf:"bytes,3,opt,name=DocumentName,proto3" json:"DocumentName,omitempty"`
	DocumentType     string                 `protobuf:"bytes,4,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"`
	DocumentUrl      string                 `protobuf:"bytes,5,opt,name=DocumentUrl,proto3" json:"DocumentUrl,omitempty"`
	FileSizeBytes    int64                  `protobuf:"varint,6,opt,name=FileSizeBytes,proto3" json:"FileSizeBytes,omitempty"`
	MimeType         string                 `protobuf:"bytes,7,opt,name=MimeType,proto3" json:"MimeType,omitempty"`
	DocumentNumber   string                 `protobuf:"bytes,8,opt,name=DocumentNumber,proto3" json:"DocumentNumber,omitempty"`
	IssuingAuthority string                 `protobuf:"bytes,9,opt,name=IssuingAuthority,proto3" json:"IssuingAuthority,omitempty"`
	Status           string                 `protobuf:"bytes,10,opt,name=Status,proto3" json:"Status,omitempty"`
	IsCurrent        bool                   `protobuf:"varint,11,opt,name=IsCurrent,proto3" json:"IsCurrent,omitempty"`
	UploadedAt       string                 `protobuf:"bytes,12,opt,name=UploadedAt,proto3" json:"UploadedAt,omitempty"`
	UpdatedAt        string                 `protobuf:"bytes,13,opt,name=UpdatedAt,proto3" json:"UpdatedAt,omitempty"`
	UserId           string                 `protobuf:"bytes,14,opt,name=UserId,proto3" json:"UserId,omitempty"`     // Propriétaire (driver) — dénormalisation migration 000005
	IssuedAt         string                 `protobuf:"bytes,15,opt,name=IssuedAt,proto3" json:"IssuedAt,omitempty"` // ISO 8601 ou "" si non défini
	ExpireAt         string                 `protobuf:"bytes,16,opt,name=ExpireAt,proto3" json:"ExpireAt,omitempty"` // ISO 8601 ou "" si non défini
	Vehicle          *VehicleInfo           `protobuf:"bytes,17,opt,name=Vehicle,proto3" json:"Vehicle,omitempty"`   // Détails du véhicule (null si non résolu)
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *VehicleDocumentResponse) Reset() {
	*x = VehicleDocumentResponse{}
	mi := &file_file_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *VehicleDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VehicleDocumentResponse) ProtoMessage() {}

func (x *VehicleDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VehicleDocumentResponse.ProtoReflect.Descriptor instead.
func (*VehicleDocumentResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{37}
}

func (x *VehicleDocumentResponse) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

func (x *VehicleDocumentResponse) GetVehicleId() string {
	if x != nil {
		return x.VehicleId
	}
	return ""
}

func (x *VehicleDocumentResponse) GetDocumentName() string {
	if x != nil {
		return x.DocumentName
	}
	return ""
}

func (x *VehicleDocumentResponse) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *VehicleDocumentResponse) GetDocumentUrl() string {
	if x != nil {
		return x.DocumentUrl
	}
	return ""
}

func (x *VehicleDocumentResponse) GetFileSizeBytes() int64 {
	if x != nil {
		return x.FileSizeBytes
	}
	return 0
}

func (x *VehicleDocumentResponse) GetMimeType() string {
	if x != nil {
		return x.MimeType
	}
	return ""
}

func (x *VehicleDocumentResponse) GetDocumentNumber() string {
	if x != nil {
		return x.DocumentNumber
	}
	return ""
}

func (x *VehicleDocumentResponse) GetIssuingAuthority() string {
	if x != nil {
		return x.IssuingAuthority
	}
	return ""
}

func (x *VehicleDocumentResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *VehicleDocumentResponse) GetIsCurrent() bool {
	if x != nil {
		return x.IsCurrent
	}
	return false
}

func (x *VehicleDocumentResponse) GetUploadedAt() string {
	if x != nil {
		return x.UploadedAt
	}
	return ""
}

func (x *VehicleDocumentResponse) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *VehicleDocumentResponse) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *VehicleDocumentResponse) GetIssuedAt() string {
	if x != nil {
		return x.IssuedAt
	}
	return ""
}

func (x *VehicleDocumentResponse) GetExpireAt() string {
	if x != nil {
		return x.ExpireAt
	}
	return ""
}

func (x *VehicleDocumentResponse) GetVehicle() *VehicleInfo {
	if x != nil {
		return x.Vehicle
	}
	return nil
}

type GetUserDocumentsResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Documents     []*UserDocumentResponse `protobuf:"bytes,1,rep,name=documents,proto3" json:"documents,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserDocumentsResponse) Reset() {
	*x = GetUserDocumentsResponse{}
	mi := &file_file_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserDocumentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserDocumentsResponse) ProtoMessage() {}

func (x *GetUserDocumentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserDocumentsResponse.ProtoReflect.Descriptor instead.
func (*GetUserDocumentsResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{38}
}

func (x *GetUserDocumentsResponse) GetDocuments() []*UserDocumentResponse {
	if x != nil {
		return x.Documents
	}
	return nil
}

type GetVehicleDocumentsResponse struct {
	state         protoimpl.MessageState     `protogen:"open.v1"`
	Documents     []*VehicleDocumentResponse `protobuf:"bytes,1,rep,name=documents,proto3" json:"documents,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetVehicleDocumentsResponse) Reset() {
	*x = GetVehicleDocumentsResponse{}
	mi := &file_file_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetVehicleDocumentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetVehicleDocumentsResponse) ProtoMessage() {}

func (x *GetVehicleDocumentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetVehicleDocumentsResponse.ProtoReflect.Descriptor instead.
func (*GetVehicleDocumentsResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{39}
}

func (x *GetVehicleDocumentsResponse) GetDocuments() []*VehicleDocumentResponse {
	if x != nil {
		return x.Documents
	}
	return nil
}

type OperationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OperationResponse) Reset() {
	*x = OperationResponse{}
	mi := &file_file_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OperationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OperationResponse) ProtoMessage() {}

func (x *OperationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OperationResponse.ProtoReflect.Descriptor instead.
func (*OperationResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{40}
}

func (x *OperationResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

type DocumentReviewResponse struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ReviewId          string                 `protobuf:"bytes,1,opt,name=ReviewId,proto3" json:"ReviewId,omitempty"`
	UserDocumentId    string                 `protobuf:"bytes,2,opt,name=UserDocumentId,proto3" json:"UserDocumentId,omitempty"`
	VehicleDocumentId string                 `protobuf:"bytes,3,opt,name=VehicleDocumentId,proto3" json:"VehicleDocumentId,omitempty"`
	// Retry / versioning
	AttemptNumber    int32  `protobuf:"varint,11,opt,name=AttemptNumber,proto3" json:"AttemptNumber,omitempty"`
	PreviousReviewId string `protobuf:"bytes,12,opt,name=PreviousReviewId,proto3" json:"PreviousReviewId,omitempty"`
	// Décision
	Status           string `protobuf:"bytes,13,opt,name=Status,proto3" json:"Status,omitempty"`
	Decision         string `protobuf:"bytes,14,opt,name=Decision,proto3" json:"Decision,omitempty"`
	ReasonRejection  string `protobuf:"bytes,15,opt,name=ReasonRejection,proto3" json:"ReasonRejection,omitempty"`
	RejectionDetails string `protobuf:"bytes,16,opt,name=RejectionDetails,proto3" json:"RejectionDetails,omitempty"`
	// Réviseur
	ReviewedBy    string `protobuf:"bytes,17,opt,name=ReviewedBy,proto3" json:"ReviewedBy,omitempty"`
	ReviewType    string `protobuf:"bytes,18,opt,name=ReviewType,proto3" json:"ReviewType,omitempty"`
	ReviewedAt    string `protobuf:"bytes,19,opt,name=ReviewedAt,proto3" json:"ReviewedAt,omitempty"` // ISO 8601
	Notes         string `protobuf:"bytes,20,opt,name=Notes,proto3" json:"Notes,omitempty"`
	ExtractedData []byte `protobuf:"bytes,21,opt,name=ExtractedData,proto3" json:"ExtractedData,omitempty"` // JSON
	// Timestamps
	SubmittedAt string `protobuf:"bytes,22,opt,name=SubmittedAt,proto3" json:"SubmittedAt,omitempty"` // ISO 8601
	UpdatedAt   string `protobuf:"bytes,23,opt,name=UpdatedAt,proto3" json:"UpdatedAt,omitempty"`     // ISO 8601
	// Dénormalisation (migration 000008) — toujours présents
	UserId       string `protobuf:"bytes,24,opt,name=UserId,proto3" json:"UserId,omitempty"`             // Propriétaire de la review
	DocumentType string `protobuf:"bytes,25,opt,name=DocumentType,proto3" json:"DocumentType,omitempty"` // Type de document
	// Recto-verso + historique (migration 000010)
	SecondUserDocumentId string `protobuf:"bytes,26,opt,name=SecondUserDocumentId,proto3" json:"SecondUserDocumentId,omitempty"` // verso pour les documents recto-verso
	LogicalDocumentType  string `protobuf:"bytes,27,opt,name=LogicalDocumentType,proto3" json:"LogicalDocumentType,omitempty"`   // idCard, driverLicence, passport…
	// Timestamps créé/modifié
	CreatedAt     string `protobuf:"bytes,28,opt,name=CreatedAt,proto3" json:"CreatedAt,omitempty"` // ISO 8601
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentReviewResponse) Reset() {
	*x = DocumentReviewResponse{}
	mi := &file_file_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentReviewResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentReviewResponse) ProtoMessage() {}

func (x *DocumentReviewResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentReviewResponse.ProtoReflect.Descriptor instead.
func (*DocumentReviewResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{41}
}

func (x *DocumentReviewResponse) GetReviewId() string {
	if x != nil {
		return x.ReviewId
	}
	return ""
}

func (x *DocumentReviewResponse) GetUserDocumentId() string {
	if x != nil {
		return x.UserDocumentId
	}
	return ""
}

func (x *DocumentReviewResponse) GetVehicleDocumentId() string {
	if x != nil {
		return x.VehicleDocumentId
	}
	return ""
}

func (x *DocumentReviewResponse) GetAttemptNumber() int32 {
	if x != nil {
		return x.AttemptNumber
	}
	return 0
}

func (x *DocumentReviewResponse) GetPreviousReviewId() string {
	if x != nil {
		return x.PreviousReviewId
	}
	return ""
}

func (x *DocumentReviewResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *DocumentReviewResponse) GetDecision() string {
	if x != nil {
		return x.Decision
	}
	return ""
}

func (x *DocumentReviewResponse) GetReasonRejection() string {
	if x != nil {
		return x.ReasonRejection
	}
	return ""
}

func (x *DocumentReviewResponse) GetRejectionDetails() string {
	if x != nil {
		return x.RejectionDetails
	}
	return ""
}

func (x *DocumentReviewResponse) GetReviewedBy() string {
	if x != nil {
		return x.ReviewedBy
	}
	return ""
}

func (x *DocumentReviewResponse) GetReviewType() string {
	if x != nil {
		return x.ReviewType
	}
	return ""
}

func (x *DocumentReviewResponse) GetReviewedAt() string {
	if x != nil {
		return x.ReviewedAt
	}
	return ""
}

func (x *DocumentReviewResponse) GetNotes() string {
	if x != nil {
		return x.Notes
	}
	return ""
}

func (x *DocumentReviewResponse) GetExtractedData() []byte {
	if x != nil {
		return x.ExtractedData
	}
	return nil
}

func (x *DocumentReviewResponse) GetSubmittedAt() string {
	if x != nil {
		return x.SubmittedAt
	}
	return ""
}

func (x *DocumentReviewResponse) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *DocumentReviewResponse) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *DocumentReviewResponse) GetDocumentType() string {
	if x != nil {
		return x.DocumentType
	}
	return ""
}

func (x *DocumentReviewResponse) GetSecondUserDocumentId() string {
	if x != nil {
		return x.SecondUserDocumentId
	}
	return ""
}

func (x *DocumentReviewResponse) GetLogicalDocumentType() string {
	if x != nil {
		return x.LogicalDocumentType
	}
	return ""
}

func (x *DocumentReviewResponse) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

type GetDocumentReviewsResponse struct {
	state         protoimpl.MessageState    `protogen:"open.v1"`
	Reviews       []*DocumentReviewResponse `protobuf:"bytes,1,rep,name=Reviews,proto3" json:"Reviews,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentReviewsResponse) Reset() {
	*x = GetDocumentReviewsResponse{}
	mi := &file_file_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentReviewsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentReviewsResponse) ProtoMessage() {}

func (x *GetDocumentReviewsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentReviewsResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentReviewsResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{42}
}

func (x *GetDocumentReviewsResponse) GetReviews() []*DocumentReviewResponse {
	if x != nil {
		return x.Reviews
	}
	return nil
}

type HealthResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Status        string                 `protobuf:"bytes,1,opt,name=Status,proto3" json:"Status,omitempty"`
	Version       string                 `protobuf:"bytes,2,opt,name=Version,proto3" json:"Version,omitempty"`
	Timestamp     int64                  `protobuf:"varint,3,opt,name=Timestamp,proto3" json:"Timestamp,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HealthResponse) Reset() {
	*x = HealthResponse{}
	mi := &file_file_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HealthResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HealthResponse) ProtoMessage() {}

func (x *HealthResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HealthResponse.ProtoReflect.Descriptor instead.
func (*HealthResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{43}
}

func (x *HealthResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *HealthResponse) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *HealthResponse) GetTimestamp() int64 {
	if x != nil {
		return x.Timestamp
	}
	return 0
}

type DeleteAllUserFilesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserID        string                 `protobuf:"bytes,1,opt,name=UserID,proto3" json:"UserID,omitempty"` // UserID interne (user-service)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteAllUserFilesRequest) Reset() {
	*x = DeleteAllUserFilesRequest{}
	mi := &file_file_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteAllUserFilesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteAllUserFilesRequest) ProtoMessage() {}

func (x *DeleteAllUserFilesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteAllUserFilesRequest.ProtoReflect.Descriptor instead.
func (*DeleteAllUserFilesRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{44}
}

func (x *DeleteAllUserFilesRequest) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

type DeleteAllUserFilesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=Success,proto3" json:"Success,omitempty"`
	ErrorMessage  string                 `protobuf:"bytes,2,opt,name=ErrorMessage,proto3" json:"ErrorMessage,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteAllUserFilesResponse) Reset() {
	*x = DeleteAllUserFilesResponse{}
	mi := &file_file_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteAllUserFilesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteAllUserFilesResponse) ProtoMessage() {}

func (x *DeleteAllUserFilesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteAllUserFilesResponse.ProtoReflect.Descriptor instead.
func (*DeleteAllUserFilesResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{45}
}

func (x *DeleteAllUserFilesResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *DeleteAllUserFilesResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

type GetDocumentReviewHistoryRequest struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	UserId              string                 `protobuf:"bytes,1,opt,name=UserId,proto3" json:"UserId,omitempty"`                           // Propriétaire des documents
	LogicalDocumentType string                 `protobuf:"bytes,2,opt,name=LogicalDocumentType,proto3" json:"LogicalDocumentType,omitempty"` // idCard | driverLicence | passport | insurance | registrationCard
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *GetDocumentReviewHistoryRequest) Reset() {
	*x = GetDocumentReviewHistoryRequest{}
	mi := &file_file_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentReviewHistoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentReviewHistoryRequest) ProtoMessage() {}

func (x *GetDocumentReviewHistoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentReviewHistoryRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentReviewHistoryRequest) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{46}
}

func (x *GetDocumentReviewHistoryRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *GetDocumentReviewHistoryRequest) GetLogicalDocumentType() string {
	if x != nil {
		return x.LogicalDocumentType
	}
	return ""
}

type GetDocumentReviewHistoryResponse struct {
	state         protoimpl.MessageState    `protogen:"open.v1"`
	Reviews       []*DocumentReviewResponse `protobuf:"bytes,1,rep,name=Reviews,proto3" json:"Reviews,omitempty"` // Triées par date décroissante (plus récente en premier)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentReviewHistoryResponse) Reset() {
	*x = GetDocumentReviewHistoryResponse{}
	mi := &file_file_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentReviewHistoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentReviewHistoryResponse) ProtoMessage() {}

func (x *GetDocumentReviewHistoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_file_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentReviewHistoryResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentReviewHistoryResponse) Descriptor() ([]byte, []int) {
	return file_file_proto_rawDescGZIP(), []int{47}
}

func (x *GetDocumentReviewHistoryResponse) GetReviews() []*DocumentReviewResponse {
	if x != nil {
		return x.Reviews
	}
	return nil
}

var File_file_proto protoreflect.FileDescriptor

const file_file_proto_rawDesc = "" +
	"\n" +
	"\n" +
	"file.proto\x12\x04file\x1a\x1cgoogle/api/annotations.proto\"\xa0\x01\n" +
	"\x12VehicleDocMetadata\x12&\n" +
	"\x0eDocumentNumber\x18\x01 \x01(\tR\x0eDocumentNumber\x12\x1a\n" +
	"\bIssuedAt\x18\x02 \x01(\tR\bIssuedAt\x12\x1a\n" +
	"\bExpireAt\x18\x03 \x01(\tR\bExpireAt\x12*\n" +
	"\x10IssuingAuthority\x18\x04 \x01(\tR\x10IssuingAuthority\"\x8d\x04\n" +
	"\x1dUploadVehicleDocumentsRequest\x12\x16\n" +
	"\x06UserID\x18\x01 \x01(\tR\x06UserID\x12\x1c\n" +
	"\tVehicleID\x18\x02 \x01(\tR\tVehicleID\x12.\n" +
	"\x12DriverLicenceRecto\x18\t \x01(\fR\x12DriverLicenceRecto\x12.\n" +
	"\x12DriverLicenceVerso\x18\n" +
	" \x01(\fR\x12DriverLicenceVerso\x12\x1c\n" +
	"\tAssurance\x18\x04 \x01(\fR\tAssurance\x120\n" +
	"\x13VehicleRegistration\x18\x05 \x01(\fR\x13VehicleRegistration\x12N\n" +
	"\x15DriverLicenceMetadata\x18\x06 \x01(\v2\x18.file.VehicleDocMetadataR\x15DriverLicenceMetadata\x12F\n" +
	"\x11AssuranceMetadata\x18\a \x01(\v2\x18.file.VehicleDocMetadataR\x11AssuranceMetadata\x12T\n" +
	"\x18RegistrationCardMetadata\x18\b \x01(\v2\x18.file.VehicleDocMetadataR\x18RegistrationCardMetadataJ\x04\b\x03\x10\x04R\x12DriverLicenceImage\"\x94\x01\n" +
	"\x1eUploadVehicleDocumentsResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\x12\"\n" +
	"\fErrorMessage\x18\x02 \x01(\tR\fErrorMessage\x124\n" +
	"\tDocuments\x18\x03 \x03(\v2\x16.file.UploadedDocumentR\tDocuments\"\xce\x01\n" +
	"\x10UploadedDocument\x12\x1e\n" +
	"\n" +
	"DocumentID\x18\x01 \x01(\tR\n" +
	"DocumentID\x12 \n" +
	"\vDocumentURL\x18\x02 \x01(\tR\vDocumentURL\x12\"\n" +
	"\fDocumentType\x18\x03 \x01(\tR\fDocumentType\x12\"\n" +
	"\fDocumentName\x18\x04 \x01(\tR\fDocumentName\x120\n" +
	"\x13LogicalDocumentType\x18\x05 \x01(\tR\x13LogicalDocumentType\"-\n" +
	"\x13UploadSelfieRequest\x12\x16\n" +
	"\x06Selfie\x18\x01 \x01(\fR\x06Selfie\"\x88\x01\n" +
	"\x14UploadSelfieResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\x12\"\n" +
	"\fErrorMessage\x18\x02 \x01(\tR\fErrorMessage\x122\n" +
	"\bDocument\x18\x03 \x01(\v2\x16.file.UploadedDocumentR\bDocument\"C\n" +
	"\x11DeleteFileRequest\x12\x16\n" +
	"\x06UserID\x18\x01 \x01(\tR\x06UserID\x12\x16\n" +
	"\x06FileID\x18\x02 \x01(\tR\x06FileID\"R\n" +
	"\x12DeleteFileResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\x12\"\n" +
	"\fErrorMessage\x18\x02 \x01(\tR\fErrorMessage\"D\n" +
	"\x12GetDocumentRequest\x12\x16\n" +
	"\x06FileID\x18\x01 \x01(\tR\x06FileID\x12\x16\n" +
	"\x06UserID\x18\x02 \x01(\tR\x06UserID\"\x92\x01\n" +
	"\fDocumentFile\x12\x16\n" +
	"\x06FileID\x18\x01 \x01(\tR\x06FileID\x12\x18\n" +
	"\aFileURL\x18\x02 \x01(\tR\aFileURL\x12\x1a\n" +
	"\bFileType\x18\x03 \x01(\tR\bFileType\x124\n" +
	"\x15PresignedUrlExpiresAt\x18\x04 \x01(\tR\x15PresignedUrlExpiresAt\"a\n" +
	"\x13GetDocumentResponse\x12\"\n" +
	"\fErrorMessage\x18\x01 \x01(\tR\fErrorMessage\x12&\n" +
	"\x04File\x18\x02 \x01(\v2\x12.file.DocumentFileR\x04File\"\xed\x01\n" +
	"\x15ChangeDocumentRequest\x12\x16\n" +
	"\x06UserID\x18\x01 \x01(\tR\x06UserID\x12\x16\n" +
	"\x06FileID\x18\x02 \x01(\tR\x06FileID\x12 \n" +
	"\vNewDocument\x18\x03 \x01(\fR\vNewDocument\x12&\n" +
	"\x0eDocumentNumber\x18\x04 \x01(\tR\x0eDocumentNumber\x12\x1a\n" +
	"\bIssuedAt\x18\x05 \x01(\tR\bIssuedAt\x12\x1a\n" +
	"\bExpireAt\x18\x06 \x01(\tR\bExpireAt\x12\"\n" +
	"\fIssuingPlace\x18\a \x01(\tR\fIssuingPlace\"\x8a\x01\n" +
	"\x16ChangeDocumentResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\x12\"\n" +
	"\fErrorMessage\x18\x02 \x01(\tR\fErrorMessage\x122\n" +
	"\bDocument\x18\x03 \x01(\v2\x16.file.UploadedDocumentR\bDocument\"\xb8\x03\n" +
	"\x17UploadIdDocumentRequest\x12\x16\n" +
	"\x06UserID\x18\x01 \x01(\tR\x06UserID\x12\"\n" +
	"\fDocumentType\x18\x02 \x01(\tR\fDocumentType\x12 \n" +
	"\vIDCardRecto\x18\x03 \x01(\fR\vIDCardRecto\x12 \n" +
	"\vIDCardVerso\x18\x04 \x01(\fR\vIDCardVerso\x12.\n" +
	"\x12DriverLicenceRecto\x18\f \x01(\fR\x12DriverLicenceRecto\x12.\n" +
	"\x12DriverLicenceVerso\x18\r \x01(\fR\x12DriverLicenceVerso\x12\x1a\n" +
	"\bPassport\x18\a \x01(\fR\bPassport\x12&\n" +
	"\x0eDocumentNumber\x18\b \x01(\tR\x0eDocumentNumber\x12\x1a\n" +
	"\bIssuedAt\x18\t \x01(\tR\bIssuedAt\x12\x1a\n" +
	"\bExpireAt\x18\n" +
	" \x01(\tR\bExpireAt\x12&\n" +
	"\x0eIssuingCountry\x18\v \x01(\tR\x0eIssuingCountryJ\x04\b\x05\x10\x06J\x04\b\x06\x10\aR\rDriverLicence\"\x8e\x01\n" +
	"\x18UploadIdDocumentResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\x12\"\n" +
	"\fErrorMessage\x18\x02 \x01(\tR\fErrorMessage\x124\n" +
	"\tDocuments\x18\x03 \x03(\v2\x16.file.UploadedDocumentR\tDocuments\"u\n" +
	"\x19UploadUserDocumentRequest\x128\n" +
	"\bmetadata\x18\x01 \x01(\v2\x1a.file.UserDocumentMetadataH\x00R\bmetadata\x12\x16\n" +
	"\x05Chunk\x18\x02 \x01(\fH\x00R\x05ChunkB\x06\n" +
	"\x04data\"\xc0\x02\n" +
	"\x14UserDocumentMetadata\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\x12\"\n" +
	"\fDocumentName\x18\x02 \x01(\tR\fDocumentName\x12\"\n" +
	"\fDocumentType\x18\x03 \x01(\tR\fDocumentType\x12\x1a\n" +
	"\bMimeType\x18\x04 \x01(\tR\bMimeType\x12$\n" +
	"\rFileSizeBytes\x18\x05 \x01(\x03R\rFileSizeBytes\x12&\n" +
	"\x0eDocumentNumber\x18\x06 \x01(\tR\x0eDocumentNumber\x12&\n" +
	"\x0eIssuingCountry\x18\a \x01(\tR\x0eIssuingCountry\x12\x1a\n" +
	"\bIssuedAt\x18\b \x01(\tR\bIssuedAt\x12\x1a\n" +
	"\bExpireAt\x18\t \x01(\tR\bExpireAt\"{\n" +
	"\x1cUploadVehicleDocumentRequest\x12;\n" +
	"\bmetadata\x18\x01 \x01(\v2\x1d.file.VehicleDocumentMetadataH\x00R\bmetadata\x12\x16\n" +
	"\x05Chunk\x18\x02 \x01(\fH\x00R\x05ChunkB\x06\n" +
	"\x04data\"\xcd\x02\n" +
	"\x17VehicleDocumentMetadata\x12\x1c\n" +
	"\tVehicleId\x18\x01 \x01(\tR\tVehicleId\x12\"\n" +
	"\fDocumentName\x18\x02 \x01(\tR\fDocumentName\x12\"\n" +
	"\fDocumentType\x18\x03 \x01(\tR\fDocumentType\x12\x1a\n" +
	"\bMimeType\x18\x04 \x01(\tR\bMimeType\x12$\n" +
	"\rFileSizeBytes\x18\x05 \x01(\x03R\rFileSizeBytes\x12&\n" +
	"\x0eDocumentNumber\x18\x06 \x01(\tR\x0eDocumentNumber\x12*\n" +
	"\x10IssuingAuthority\x18\a \x01(\tR\x10IssuingAuthority\x12\x1a\n" +
	"\bIssuedAt\x18\b \x01(\tR\bIssuedAt\x12\x1a\n" +
	"\bExpireAt\x18\t \x01(\tR\bExpireAt\"1\n" +
	"\x17GetUserDocumentsRequest\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\"`\n" +
	"\x16GetDocumentByIDRequest\x12\x1e\n" +
	"\n" +
	"DocumentId\x18\x01 \x01(\tR\n" +
	"DocumentId\x12&\n" +
	"\x0ePresignTTLSecs\x18\x02 \x01(\x03R\x0ePresignTTLSecs\"[\n" +
	"\x1dGetCurrentUserDocumentRequest\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\x12\"\n" +
	"\fDocumentType\x18\x02 \x01(\tR\fDocumentType\":\n" +
	"\x1aGetVehicleDocumentsRequest\x12\x1c\n" +
	"\tVehicleId\x18\x01 \x01(\tR\tVehicleId\"<\n" +
	"\"GetVehicleDocumentsByUserIDRequest\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\"5\n" +
	"\x17ListKycDocumentsRequest\x12\x1a\n" +
	"\bStatuses\x18\x01 \x03(\tR\bStatuses\"\xfb\x01\n" +
	"\vKycDocument\x12\x1e\n" +
	"\n" +
	"DocumentId\x18\x01 \x01(\tR\n" +
	"DocumentId\x12\x16\n" +
	"\x06UserId\x18\x02 \x01(\tR\x06UserId\x12\x1c\n" +
	"\tVehicleId\x18\x03 \x01(\tR\tVehicleId\x12\"\n" +
	"\fDocumentType\x18\x04 \x01(\tR\fDocumentType\x12\x16\n" +
	"\x06Status\x18\x05 \x01(\tR\x06Status\x12\x1c\n" +
	"\tOwnerKind\x18\x06 \x01(\tR\tOwnerKind\x12\x1c\n" +
	"\tUpdatedAt\x18\a \x01(\tR\tUpdatedAt\x12\x1e\n" +
	"\n" +
	"UploadedAt\x18\b \x01(\tR\n" +
	"UploadedAt\"K\n" +
	"\x18ListKycDocumentsResponse\x12/\n" +
	"\tDocuments\x18\x01 \x03(\v2\x11.file.KycDocumentR\tDocuments\"7\n" +
	"\x15DeleteDocumentRequest\x12\x1e\n" +
	"\n" +
	"DocumentId\x18\x01 \x01(\tR\n" +
	"DocumentId\"\xbd\x06\n" +
	"\x1bCreateDocumentReviewRequest\x12&\n" +
	"\x0eUserDocumentId\x18\x01 \x01(\tR\x0eUserDocumentId\x12,\n" +
	"\x11VehicleDocumentId\x18\x02 \x01(\tR\x11VehicleDocumentId\x12$\n" +
	"\rAttemptNumber\x18\n" +
	" \x01(\x05R\rAttemptNumber\x12*\n" +
	"\x10PreviousReviewId\x18\v \x01(\tR\x10PreviousReviewId\x12\x16\n" +
	"\x06Status\x18\f \x01(\tR\x06Status\x12\x1a\n" +
	"\bDecision\x18\r \x01(\tR\bDecision\x12(\n" +
	"\x0fReasonRejection\x18\x0e \x01(\tR\x0fReasonRejection\x12*\n" +
	"\x10RejectionDetails\x18\x0f \x01(\tR\x10RejectionDetails\x12\x1e\n" +
	"\n" +
	"ReviewedBy\x18\x10 \x01(\tR\n" +
	"ReviewedBy\x12\x1e\n" +
	"\n" +
	"ReviewType\x18\x11 \x01(\tR\n" +
	"ReviewType\x12\x14\n" +
	"\x05Notes\x18\x12 \x01(\tR\x05Notes\x12$\n" +
	"\rExtractedData\x18\x13 \x01(\fR\rExtractedData\x12 \n" +
	"\vSubmittedAt\x18\x14 \x01(\tR\vSubmittedAt\x12\x16\n" +
	"\x06UserId\x18\x15 \x01(\tR\x06UserId\x12\"\n" +
	"\fDocumentType\x18\x16 \x01(\tR\fDocumentType\x122\n" +
	"\x14SecondUserDocumentId\x18\x17 \x01(\tR\x14SecondUserDocumentId\x120\n" +
	"\x13LogicalDocumentType\x18\x18 \x01(\tR\x13LogicalDocumentTypeJ\x04\b\x03\x10\x04J\x04\b\x04\x10\x05J\x04\b\x05\x10\x06J\x04\b\x06\x10\aJ\x04\b\a\x10\bJ\x04\b\b\x10\tJ\x04\b\t\x10\n" +
	"R\x10PersonaInquiryIdR\x11PersonaTemplateIdR\x13PersonaSessionTokenR\x10SessionExpiresAtR\x10WebhookEventTypeR\x11WebhookReceivedAtR\x11PersonaRawPayload\"q\n" +
	"\x19GetDocumentReviewsRequest\x12&\n" +
	"\x0eUserDocumentId\x18\x01 \x01(\tR\x0eUserDocumentId\x12,\n" +
	"\x11VehicleDocumentId\x18\x02 \x01(\tR\x11VehicleDocumentId\":\n" +
	"\x1cGetDocumentReviewByIDRequest\x12\x1a\n" +
	"\bReviewId\x18\x01 \x01(\tR\bReviewId\";\n" +
	"!GetDocumentReviewsByUserIDRequest\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\"\xec\x03\n" +
	"\x1bUpdateDocumentReviewRequest\x12\x1a\n" +
	"\bReviewId\x18\x01 \x01(\tR\bReviewId\x12\x16\n" +
	"\x06Status\x18\a \x01(\tR\x06Status\x12\x1a\n" +
	"\bDecision\x18\b \x01(\tR\bDecision\x12(\n" +
	"\x0fReasonRejection\x18\t \x01(\tR\x0fReasonRejection\x12*\n" +
	"\x10RejectionDetails\x18\n" +
	" \x01(\tR\x10RejectionDetails\x12\x1e\n" +
	"\n" +
	"ReviewedBy\x18\v \x01(\tR\n" +
	"ReviewedBy\x12\x1e\n" +
	"\n" +
	"ReviewType\x18\f \x01(\tR\n" +
	"ReviewType\x12\x14\n" +
	"\x05Notes\x18\r \x01(\tR\x05Notes\x12 \n" +
	"\vSubmittedAt\x18\x0e \x01(\tR\vSubmittedAt\x122\n" +
	"\x14SecondUserDocumentId\x18\x0f \x01(\tR\x14SecondUserDocumentIdJ\x04\b\x02\x10\x03J\x04\b\x03\x10\x04J\x04\b\x04\x10\x05J\x04\b\x05\x10\x06J\x04\b\x06\x10\aR\x13PersonaSessionTokenR\x10SessionExpiresAtR\x10WebhookEventTypeR\x11WebhookReceivedAtR\x11PersonaRawPayload\"\x98\x01\n" +
	"\x1aListDocumentReviewsRequest\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\x12\x16\n" +
	"\x06Status\x18\x02 \x01(\tR\x06Status\x12\x1a\n" +
	"\bDecision\x18\x03 \x01(\tR\bDecision\x12\x12\n" +
	"\x04Page\x18\x04 \x01(\x05R\x04Page\x12\x1a\n" +
	"\bPageSize\x18\x05 \x01(\x05R\bPageSize\"\x0f\n" +
	"\rHealthRequest\"\xaa\x04\n" +
	"\x14UserDocumentResponse\x12\x1e\n" +
	"\n" +
	"DocumentId\x18\x01 \x01(\tR\n" +
	"DocumentId\x12\x16\n" +
	"\x06UserId\x18\x02 \x01(\tR\x06UserId\x12\"\n" +
	"\fDocumentName\x18\x03 \x01(\tR\fDocumentName\x12\"\n" +
	"\fDocumentType\x18\x04 \x01(\tR\fDocumentType\x12 \n" +
	"\vDocumentUrl\x18\x05 \x01(\tR\vDocumentUrl\x12$\n" +
	"\rFileSizeBytes\x18\x06 \x01(\x03R\rFileSizeBytes\x12\x1a\n" +
	"\bMimeType\x18\a \x01(\tR\bMimeType\x12&\n" +
	"\x0eDocumentNumber\x18\b \x01(\tR\x0eDocumentNumber\x12&\n" +
	"\x0eIssuingCountry\x18\t \x01(\tR\x0eIssuingCountry\x12\x16\n" +
	"\x06Status\x18\n" +
	" \x01(\tR\x06Status\x12\x1c\n" +
	"\tIsCurrent\x18\v \x01(\bR\tIsCurrent\x12\x1e\n" +
	"\n" +
	"UploadedAt\x18\f \x01(\tR\n" +
	"UploadedAt\x12\x1c\n" +
	"\tUpdatedAt\x18\r \x01(\tR\tUpdatedAt\x12\x1c\n" +
	"\tExpiredAt\x18\x0e \x01(\tR\tExpiredAt\x120\n" +
	"\x13LogicalDocumentType\x18\x0f \x01(\tR\x13LogicalDocumentType\x12\x1a\n" +
	"\bIssuedAt\x18\x10 \x01(\tR\bIssuedAt\"\xe1\x01\n" +
	"\vVehicleInfo\x12\x1c\n" +
	"\tVehicleId\x18\x01 \x01(\tR\tVehicleId\x12\x14\n" +
	"\x05Brand\x18\x02 \x01(\tR\x05Brand\x12\x1e\n" +
	"\n" +
	"BrandModel\x18\x03 \x01(\tR\n" +
	"BrandModel\x12\x14\n" +
	"\x05Color\x18\x04 \x01(\tR\x05Color\x12\"\n" +
	"\fLicencePlate\x18\x05 \x01(\tR\fLicencePlate\x12$\n" +
	"\rNumberOfSeats\x18\x06 \x01(\x05R\rNumberOfSeats\x12\x1e\n" +
	"\n" +
	"IsVerified\x18\a \x01(\bR\n" +
	"IsVerified\"\xc8\x04\n" +
	"\x17VehicleDocumentResponse\x12\x1e\n" +
	"\n" +
	"DocumentId\x18\x01 \x01(\tR\n" +
	"DocumentId\x12\x1c\n" +
	"\tVehicleId\x18\x02 \x01(\tR\tVehicleId\x12\"\n" +
	"\fDocumentName\x18\x03 \x01(\tR\fDocumentName\x12\"\n" +
	"\fDocumentType\x18\x04 \x01(\tR\fDocumentType\x12 \n" +
	"\vDocumentUrl\x18\x05 \x01(\tR\vDocumentUrl\x12$\n" +
	"\rFileSizeBytes\x18\x06 \x01(\x03R\rFileSizeBytes\x12\x1a\n" +
	"\bMimeType\x18\a \x01(\tR\bMimeType\x12&\n" +
	"\x0eDocumentNumber\x18\b \x01(\tR\x0eDocumentNumber\x12*\n" +
	"\x10IssuingAuthority\x18\t \x01(\tR\x10IssuingAuthority\x12\x16\n" +
	"\x06Status\x18\n" +
	" \x01(\tR\x06Status\x12\x1c\n" +
	"\tIsCurrent\x18\v \x01(\bR\tIsCurrent\x12\x1e\n" +
	"\n" +
	"UploadedAt\x18\f \x01(\tR\n" +
	"UploadedAt\x12\x1c\n" +
	"\tUpdatedAt\x18\r \x01(\tR\tUpdatedAt\x12\x16\n" +
	"\x06UserId\x18\x0e \x01(\tR\x06UserId\x12\x1a\n" +
	"\bIssuedAt\x18\x0f \x01(\tR\bIssuedAt\x12\x1a\n" +
	"\bExpireAt\x18\x10 \x01(\tR\bExpireAt\x12+\n" +
	"\aVehicle\x18\x11 \x01(\v2\x11.file.VehicleInfoR\aVehicle\"T\n" +
	"\x18GetUserDocumentsResponse\x128\n" +
	"\tdocuments\x18\x01 \x03(\v2\x1a.file.UserDocumentResponseR\tdocuments\"Z\n" +
	"\x1bGetVehicleDocumentsResponse\x12;\n" +
	"\tdocuments\x18\x01 \x03(\v2\x1d.file.VehicleDocumentResponseR\tdocuments\"-\n" +
	"\x11OperationResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\"\xb0\a\n" +
	"\x16DocumentReviewResponse\x12\x1a\n" +
	"\bReviewId\x18\x01 \x01(\tR\bReviewId\x12&\n" +
	"\x0eUserDocumentId\x18\x02 \x01(\tR\x0eUserDocumentId\x12,\n" +
	"\x11VehicleDocumentId\x18\x03 \x01(\tR\x11VehicleDocumentId\x12$\n" +
	"\rAttemptNumber\x18\v \x01(\x05R\rAttemptNumber\x12*\n" +
	"\x10PreviousReviewId\x18\f \x01(\tR\x10PreviousReviewId\x12\x16\n" +
	"\x06Status\x18\r \x01(\tR\x06Status\x12\x1a\n" +
	"\bDecision\x18\x0e \x01(\tR\bDecision\x12(\n" +
	"\x0fReasonRejection\x18\x0f \x01(\tR\x0fReasonRejection\x12*\n" +
	"\x10RejectionDetails\x18\x10 \x01(\tR\x10RejectionDetails\x12\x1e\n" +
	"\n" +
	"ReviewedBy\x18\x11 \x01(\tR\n" +
	"ReviewedBy\x12\x1e\n" +
	"\n" +
	"ReviewType\x18\x12 \x01(\tR\n" +
	"ReviewType\x12\x1e\n" +
	"\n" +
	"ReviewedAt\x18\x13 \x01(\tR\n" +
	"ReviewedAt\x12\x14\n" +
	"\x05Notes\x18\x14 \x01(\tR\x05Notes\x12$\n" +
	"\rExtractedData\x18\x15 \x01(\fR\rExtractedData\x12 \n" +
	"\vSubmittedAt\x18\x16 \x01(\tR\vSubmittedAt\x12\x1c\n" +
	"\tUpdatedAt\x18\x17 \x01(\tR\tUpdatedAt\x12\x16\n" +
	"\x06UserId\x18\x18 \x01(\tR\x06UserId\x12\"\n" +
	"\fDocumentType\x18\x19 \x01(\tR\fDocumentType\x122\n" +
	"\x14SecondUserDocumentId\x18\x1a \x01(\tR\x14SecondUserDocumentId\x120\n" +
	"\x13LogicalDocumentType\x18\x1b \x01(\tR\x13LogicalDocumentType\x12\x1c\n" +
	"\tCreatedAt\x18\x1c \x01(\tR\tCreatedAtJ\x04\b\x04\x10\x05J\x04\b\x05\x10\x06J\x04\b\x06\x10\aJ\x04\b\a\x10\bJ\x04\b\b\x10\tJ\x04\b\t\x10\n" +
	"J\x04\b\n" +
	"\x10\vR\x10PersonaInquiryIdR\x11PersonaTemplateIdR\x13PersonaSessionTokenR\x10SessionExpiresAtR\x10WebhookEventTypeR\x11WebhookReceivedAtR\x11PersonaRawPayload\"T\n" +
	"\x1aGetDocumentReviewsResponse\x126\n" +
	"\aReviews\x18\x01 \x03(\v2\x1c.file.DocumentReviewResponseR\aReviews\"`\n" +
	"\x0eHealthResponse\x12\x16\n" +
	"\x06Status\x18\x01 \x01(\tR\x06Status\x12\x18\n" +
	"\aVersion\x18\x02 \x01(\tR\aVersion\x12\x1c\n" +
	"\tTimestamp\x18\x03 \x01(\x03R\tTimestamp\"3\n" +
	"\x19DeleteAllUserFilesRequest\x12\x16\n" +
	"\x06UserID\x18\x01 \x01(\tR\x06UserID\"Z\n" +
	"\x1aDeleteAllUserFilesResponse\x12\x18\n" +
	"\aSuccess\x18\x01 \x01(\bR\aSuccess\x12\"\n" +
	"\fErrorMessage\x18\x02 \x01(\tR\fErrorMessage\"k\n" +
	"\x1fGetDocumentReviewHistoryRequest\x12\x16\n" +
	"\x06UserId\x18\x01 \x01(\tR\x06UserId\x120\n" +
	"\x13LogicalDocumentType\x18\x02 \x01(\tR\x13LogicalDocumentType\"Z\n" +
	" GetDocumentReviewHistoryResponse\x126\n" +
	"\aReviews\x18\x01 \x03(\v2\x1c.file.DocumentReviewResponseR\aReviews2\xa3\x13\n" +
	"\vFileService\x12S\n" +
	"\x12UploadUserDocument\x12\x1f.file.UploadUserDocumentRequest\x1a\x1a.file.UserDocumentResponse(\x01\x12\\\n" +
	"\x15UploadVehicleDocument\x12\".file.UploadVehicleDocumentRequest\x1a\x1d.file.VehicleDocumentResponse(\x01\x12{\n" +
	"\x10UploadIdDocument\x12\x1d.file.UploadIdDocumentRequest\x1a\x1e.file.UploadIdDocumentResponse\"(\x82\xd3\xe4\x93\x02\":\x01*\"\x1d/api/v1/file/uploadIdDocument\x12\x93\x01\n" +
	"\x16UploadVehicleDocuments\x12#.file.UploadVehicleDocumentsRequest\x1a$.file.UploadVehicleDocumentsResponse\".\x82\xd3\xe4\x93\x02(:\x01*\"#/api/v1/file/uploadVehicleDocuments\x12k\n" +
	"\fUploadSelfie\x12\x19.file.UploadSelfieRequest\x1a\x1a.file.UploadSelfieResponse\"$\x82\xd3\xe4\x93\x02\x1e:\x01*\"\x19/api/v1/file/uploadSelfie\x12s\n" +
	"\x0eChangeDocument\x12\x1b.file.ChangeDocumentRequest\x1a\x1c.file.ChangeDocumentResponse\"&\x82\xd3\xe4\x93\x02 :\x01*2\x1b/api/v1/file/changeDocument\x12d\n" +
	"\vGetDocument\x12\x18.file.GetDocumentRequest\x1a\x19.file.GetDocumentResponse\" \x82\xd3\xe4\x93\x02\x1a\x12\x18/api/v1/file/getDocument\x12Q\n" +
	"\x10GetUserDocuments\x12\x1d.file.GetUserDocumentsRequest\x1a\x1e.file.GetUserDocumentsResponse\x12K\n" +
	"\x0fGetUserDocument\x12\x1c.file.GetDocumentByIDRequest\x1a\x1a.file.UserDocumentResponse\x12Y\n" +
	"\x16GetCurrentUserDocument\x12#.file.GetCurrentUserDocumentRequest\x1a\x1a.file.UserDocumentResponse\x12Z\n" +
	"\x13GetVehicleDocuments\x12 .file.GetVehicleDocumentsRequest\x1a!.file.GetVehicleDocumentsResponse\x12Q\n" +
	"\x12GetVehicleDocument\x12\x1c.file.GetDocumentByIDRequest\x1a\x1d.file.VehicleDocumentResponse\x12j\n" +
	"\x1bGetVehicleDocumentsByUserID\x12(.file.GetVehicleDocumentsByUserIDRequest\x1a!.file.GetVehicleDocumentsResponse\x12Q\n" +
	"\x10ListKycDocuments\x12\x1d.file.ListKycDocumentsRequest\x1a\x1e.file.ListKycDocumentsResponse\x12c\n" +
	"\n" +
	"DeleteFile\x12\x17.file.DeleteFileRequest\x1a\x18.file.DeleteFileResponse\"\"\x82\xd3\xe4\x93\x02\x1c:\x01*\"\x17/api/v1/file/deleteFile\x12J\n" +
	"\x12DeleteUserDocument\x12\x1b.file.DeleteDocumentRequest\x1a\x17.file.OperationResponse\x12M\n" +
	"\x15DeleteVehicleDocument\x12\x1b.file.DeleteDocumentRequest\x1a\x17.file.OperationResponse\x12W\n" +
	"\x14CreateDocumentReview\x12!.file.CreateDocumentReviewRequest\x1a\x1c.file.DocumentReviewResponse\x12U\n" +
	"\x11GetDocumentReview\x12\".file.GetDocumentReviewByIDRequest\x1a\x1c.file.DocumentReviewResponse\x12W\n" +
	"\x12GetDocumentReviews\x12\x1f.file.GetDocumentReviewsRequest\x1a .file.GetDocumentReviewsResponse\x12g\n" +
	"\x1aGetDocumentReviewsByUserID\x12'.file.GetDocumentReviewsByUserIDRequest\x1a .file.GetDocumentReviewsResponse\x12W\n" +
	"\x14UpdateDocumentReview\x12!.file.UpdateDocumentReviewRequest\x1a\x1c.file.DocumentReviewResponse\x12Y\n" +
	"\x13ListDocumentReviews\x12 .file.ListDocumentReviewsRequest\x1a .file.GetDocumentReviewsResponse\x12i\n" +
	"\x18GetDocumentReviewHistory\x12%.file.GetDocumentReviewHistoryRequest\x1a&.file.GetDocumentReviewHistoryResponse\x123\n" +
	"\x06Health\x12\x13.file.HealthRequest\x1a\x14.file.HealthResponse\x12W\n" +
	"\x12DeleteAllUserFiles\x12\x1f.file.DeleteAllUserFilesRequest\x1a .file.DeleteAllUserFilesResponseBBZ@github.com/Kpeewu/tissi-mah/services/file-service/proto/gen;fileb\x06proto3"

var (
	file_file_proto_rawDescOnce sync.Once
	file_file_proto_rawDescData []byte
)

func file_file_proto_rawDescGZIP() []byte {
	file_file_proto_rawDescOnce.Do(func() {
		file_file_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_file_proto_rawDesc), len(file_file_proto_rawDesc)))
	})
	return file_file_proto_rawDescData
}

var file_file_proto_msgTypes = make([]protoimpl.MessageInfo, 48)
var file_file_proto_goTypes = []any{
	(*VehicleDocMetadata)(nil),                 // 0: file.VehicleDocMetadata
	(*UploadVehicleDocumentsRequest)(nil),      // 1: file.UploadVehicleDocumentsRequest
	(*UploadVehicleDocumentsResponse)(nil),     // 2: file.UploadVehicleDocumentsResponse
	(*UploadedDocument)(nil),                   // 3: file.UploadedDocument
	(*UploadSelfieRequest)(nil),                // 4: file.UploadSelfieRequest
	(*UploadSelfieResponse)(nil),               // 5: file.UploadSelfieResponse
	(*DeleteFileRequest)(nil),                  // 6: file.DeleteFileRequest
	(*DeleteFileResponse)(nil),                 // 7: file.DeleteFileResponse
	(*GetDocumentRequest)(nil),                 // 8: file.GetDocumentRequest
	(*DocumentFile)(nil),                       // 9: file.DocumentFile
	(*GetDocumentResponse)(nil),                // 10: file.GetDocumentResponse
	(*ChangeDocumentRequest)(nil),              // 11: file.ChangeDocumentRequest
	(*ChangeDocumentResponse)(nil),             // 12: file.ChangeDocumentResponse
	(*UploadIdDocumentRequest)(nil),            // 13: file.UploadIdDocumentRequest
	(*UploadIdDocumentResponse)(nil),           // 14: file.UploadIdDocumentResponse
	(*UploadUserDocumentRequest)(nil),          // 15: file.UploadUserDocumentRequest
	(*UserDocumentMetadata)(nil),               // 16: file.UserDocumentMetadata
	(*UploadVehicleDocumentRequest)(nil),       // 17: file.UploadVehicleDocumentRequest
	(*VehicleDocumentMetadata)(nil),            // 18: file.VehicleDocumentMetadata
	(*GetUserDocumentsRequest)(nil),            // 19: file.GetUserDocumentsRequest
	(*GetDocumentByIDRequest)(nil),             // 20: file.GetDocumentByIDRequest
	(*GetCurrentUserDocumentRequest)(nil),      // 21: file.GetCurrentUserDocumentRequest
	(*GetVehicleDocumentsRequest)(nil),         // 22: file.GetVehicleDocumentsRequest
	(*GetVehicleDocumentsByUserIDRequest)(nil), // 23: file.GetVehicleDocumentsByUserIDRequest
	(*ListKycDocumentsRequest)(nil),            // 24: file.ListKycDocumentsRequest
	(*KycDocument)(nil),                        // 25: file.KycDocument
	(*ListKycDocumentsResponse)(nil),           // 26: file.ListKycDocumentsResponse
	(*DeleteDocumentRequest)(nil),              // 27: file.DeleteDocumentRequest
	(*CreateDocumentReviewRequest)(nil),        // 28: file.CreateDocumentReviewRequest
	(*GetDocumentReviewsRequest)(nil),          // 29: file.GetDocumentReviewsRequest
	(*GetDocumentReviewByIDRequest)(nil),       // 30: file.GetDocumentReviewByIDRequest
	(*GetDocumentReviewsByUserIDRequest)(nil),  // 31: file.GetDocumentReviewsByUserIDRequest
	(*UpdateDocumentReviewRequest)(nil),        // 32: file.UpdateDocumentReviewRequest
	(*ListDocumentReviewsRequest)(nil),         // 33: file.ListDocumentReviewsRequest
	(*HealthRequest)(nil),                      // 34: file.HealthRequest
	(*UserDocumentResponse)(nil),               // 35: file.UserDocumentResponse
	(*VehicleInfo)(nil),                        // 36: file.VehicleInfo
	(*VehicleDocumentResponse)(nil),            // 37: file.VehicleDocumentResponse
	(*GetUserDocumentsResponse)(nil),           // 38: file.GetUserDocumentsResponse
	(*GetVehicleDocumentsResponse)(nil),        // 39: file.GetVehicleDocumentsResponse
	(*OperationResponse)(nil),                  // 40: file.OperationResponse
	(*DocumentReviewResponse)(nil),             // 41: file.DocumentReviewResponse
	(*GetDocumentReviewsResponse)(nil),         // 42: file.GetDocumentReviewsResponse
	(*HealthResponse)(nil),                     // 43: file.HealthResponse
	(*DeleteAllUserFilesRequest)(nil),          // 44: file.DeleteAllUserFilesRequest
	(*DeleteAllUserFilesResponse)(nil),         // 45: file.DeleteAllUserFilesResponse
	(*GetDocumentReviewHistoryRequest)(nil),    // 46: file.GetDocumentReviewHistoryRequest
	(*GetDocumentReviewHistoryResponse)(nil),   // 47: file.GetDocumentReviewHistoryResponse
}
var file_file_proto_depIdxs = []int32{
	0,  // 0: file.UploadVehicleDocumentsRequest.DriverLicenceMetadata:type_name -> file.VehicleDocMetadata
	0,  // 1: file.UploadVehicleDocumentsRequest.AssuranceMetadata:type_name -> file.VehicleDocMetadata
	0,  // 2: file.UploadVehicleDocumentsRequest.RegistrationCardMetadata:type_name -> file.VehicleDocMetadata
	3,  // 3: file.UploadVehicleDocumentsResponse.Documents:type_name -> file.UploadedDocument
	3,  // 4: file.UploadSelfieResponse.Document:type_name -> file.UploadedDocument
	9,  // 5: file.GetDocumentResponse.File:type_name -> file.DocumentFile
	3,  // 6: file.ChangeDocumentResponse.Document:type_name -> file.UploadedDocument
	3,  // 7: file.UploadIdDocumentResponse.Documents:type_name -> file.UploadedDocument
	16, // 8: file.UploadUserDocumentRequest.metadata:type_name -> file.UserDocumentMetadata
	18, // 9: file.UploadVehicleDocumentRequest.metadata:type_name -> file.VehicleDocumentMetadata
	25, // 10: file.ListKycDocumentsResponse.Documents:type_name -> file.KycDocument
	36, // 11: file.VehicleDocumentResponse.Vehicle:type_name -> file.VehicleInfo
	35, // 12: file.GetUserDocumentsResponse.documents:type_name -> file.UserDocumentResponse
	37, // 13: file.GetVehicleDocumentsResponse.documents:type_name -> file.VehicleDocumentResponse
	41, // 14: file.GetDocumentReviewsResponse.Reviews:type_name -> file.DocumentReviewResponse
	41, // 15: file.GetDocumentReviewHistoryResponse.Reviews:type_name -> file.DocumentReviewResponse
	15, // 16: file.FileService.UploadUserDocument:input_type -> file.UploadUserDocumentRequest
	17, // 17: file.FileService.UploadVehicleDocument:input_type -> file.UploadVehicleDocumentRequest
	13, // 18: file.FileService.UploadIdDocument:input_type -> file.UploadIdDocumentRequest
	1,  // 19: file.FileService.UploadVehicleDocuments:input_type -> file.UploadVehicleDocumentsRequest
	4,  // 20: file.FileService.UploadSelfie:input_type -> file.UploadSelfieRequest
	11, // 21: file.FileService.ChangeDocument:input_type -> file.ChangeDocumentRequest
	8,  // 22: file.FileService.GetDocument:input_type -> file.GetDocumentRequest
	19, // 23: file.FileService.GetUserDocuments:input_type -> file.GetUserDocumentsRequest
	20, // 24: file.FileService.GetUserDocument:input_type -> file.GetDocumentByIDRequest
	21, // 25: file.FileService.GetCurrentUserDocument:input_type -> file.GetCurrentUserDocumentRequest
	22, // 26: file.FileService.GetVehicleDocuments:input_type -> file.GetVehicleDocumentsRequest
	20, // 27: file.FileService.GetVehicleDocument:input_type -> file.GetDocumentByIDRequest
	23, // 28: file.FileService.GetVehicleDocumentsByUserID:input_type -> file.GetVehicleDocumentsByUserIDRequest
	24, // 29: file.FileService.ListKycDocuments:input_type -> file.ListKycDocumentsRequest
	6,  // 30: file.FileService.DeleteFile:input_type -> file.DeleteFileRequest
	27, // 31: file.FileService.DeleteUserDocument:input_type -> file.DeleteDocumentRequest
	27, // 32: file.FileService.DeleteVehicleDocument:input_type -> file.DeleteDocumentRequest
	28, // 33: file.FileService.CreateDocumentReview:input_type -> file.CreateDocumentReviewRequest
	30, // 34: file.FileService.GetDocumentReview:input_type -> file.GetDocumentReviewByIDRequest
	29, // 35: file.FileService.GetDocumentReviews:input_type -> file.GetDocumentReviewsRequest
	31, // 36: file.FileService.GetDocumentReviewsByUserID:input_type -> file.GetDocumentReviewsByUserIDRequest
	32, // 37: file.FileService.UpdateDocumentReview:input_type -> file.UpdateDocumentReviewRequest
	33, // 38: file.FileService.ListDocumentReviews:input_type -> file.ListDocumentReviewsRequest
	46, // 39: file.FileService.GetDocumentReviewHistory:input_type -> file.GetDocumentReviewHistoryRequest
	34, // 40: file.FileService.Health:input_type -> file.HealthRequest
	44, // 41: file.FileService.DeleteAllUserFiles:input_type -> file.DeleteAllUserFilesRequest
	35, // 42: file.FileService.UploadUserDocument:output_type -> file.UserDocumentResponse
	37, // 43: file.FileService.UploadVehicleDocument:output_type -> file.VehicleDocumentResponse
	14, // 44: file.FileService.UploadIdDocument:output_type -> file.UploadIdDocumentResponse
	2,  // 45: file.FileService.UploadVehicleDocuments:output_type -> file.UploadVehicleDocumentsResponse
	5,  // 46: file.FileService.UploadSelfie:output_type -> file.UploadSelfieResponse
	12, // 47: file.FileService.ChangeDocument:output_type -> file.ChangeDocumentResponse
	10, // 48: file.FileService.GetDocument:output_type -> file.GetDocumentResponse
	38, // 49: file.FileService.GetUserDocuments:output_type -> file.GetUserDocumentsResponse
	35, // 50: file.FileService.GetUserDocument:output_type -> file.UserDocumentResponse
	35, // 51: file.FileService.GetCurrentUserDocument:output_type -> file.UserDocumentResponse
	39, // 52: file.FileService.GetVehicleDocuments:output_type -> file.GetVehicleDocumentsResponse
	37, // 53: file.FileService.GetVehicleDocument:output_type -> file.VehicleDocumentResponse
	39, // 54: file.FileService.GetVehicleDocumentsByUserID:output_type -> file.GetVehicleDocumentsResponse
	26, // 55: file.FileService.ListKycDocuments:output_type -> file.ListKycDocumentsResponse
	7,  // 56: file.FileService.DeleteFile:output_type -> file.DeleteFileResponse
	40, // 57: file.FileService.DeleteUserDocument:output_type -> file.OperationResponse
	40, // 58: file.FileService.DeleteVehicleDocument:output_type -> file.OperationResponse
	41, // 59: file.FileService.CreateDocumentReview:output_type -> file.DocumentReviewResponse
	41, // 60: file.FileService.GetDocumentReview:output_type -> file.DocumentReviewResponse
	42, // 61: file.FileService.GetDocumentReviews:output_type -> file.GetDocumentReviewsResponse
	42, // 62: file.FileService.GetDocumentReviewsByUserID:output_type -> file.GetDocumentReviewsResponse
	41, // 63: file.FileService.UpdateDocumentReview:output_type -> file.DocumentReviewResponse
	42, // 64: file.FileService.ListDocumentReviews:output_type -> file.GetDocumentReviewsResponse
	47, // 65: file.FileService.GetDocumentReviewHistory:output_type -> file.GetDocumentReviewHistoryResponse
	43, // 66: file.FileService.Health:output_type -> file.HealthResponse
	45, // 67: file.FileService.DeleteAllUserFiles:output_type -> file.DeleteAllUserFilesResponse
	42, // [42:68] is the sub-list for method output_type
	16, // [16:42] is the sub-list for method input_type
	16, // [16:16] is the sub-list for extension type_name
	16, // [16:16] is the sub-list for extension extendee
	0,  // [0:16] is the sub-list for field type_name
}

func init() { file_file_proto_init() }
func file_file_proto_init() {
	if File_file_proto != nil {
		return
	}
	file_file_proto_msgTypes[15].OneofWrappers = []any{
		(*UploadUserDocumentRequest_Metadata)(nil),
		(*UploadUserDocumentRequest_Chunk)(nil),
	}
	file_file_proto_msgTypes[17].OneofWrappers = []any{
		(*UploadVehicleDocumentRequest_Metadata)(nil),
		(*UploadVehicleDocumentRequest_Chunk)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_file_proto_rawDesc), len(file_file_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   48,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_file_proto_goTypes,
		DependencyIndexes: file_file_proto_depIdxs,
		MessageInfos:      file_file_proto_msgTypes,
	}.Build()
	File_file_proto = out.File
	file_file_proto_goTypes = nil
	file_file_proto_depIdxs = nil
}
